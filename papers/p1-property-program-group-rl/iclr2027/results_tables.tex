...
$k=1,8,20$. Candidate validity is 100\%, and 17.075\% of the complete unranked
20-candidate pool is a strict success. These Any@$k$ values measure whether a
condition has at least one hit; they are not the candidate-level accuracies in
Table~\ref{tab:moledit-full}. Paper-facing matched hard-de-novo values
are measured on 2,000 6p/7p conditions and 40,000 unranked candidates. The
hard subset reaches 5.5\% Strict@1, 28.3\% Pass@8, and 45.6\% Pass@20 at
33.3\% candidate validity. The complete 6,000-condition curve is reported in
the accompanying result artifact; its aggregate Pass@20 is 62.5\%.

\begin{table}[t]
  \centering
  \caption{Within-method budget scaling for the promoted single-checkpoint
  state-adaptive gate. Candidate validity is measured over all unselected
  draws; Strict@1 and Pass@$k$ are averaged over conditions. Pass@$k$ is not
  comparable to candidate-level Table~\ref{tab:moledit-full}.}
  ...
  \begin{tabular}{lrrrrr}
    ...
    \midrule
    Hard de novo (6p/7p) & 2,000 & 33.3 & 5.5 & 28.3 & 45.6 \\
    MolEdit Table 1 pilot & 200 & \textbf{100.0} & \textbf{23.5} & \textbf{53.0} & \textbf{70.5} \\
    \bottomrule
  \end{tabular}
\end{table}

Applying group-relative REINFORCE to the transaction branch is a negative
ablation. Table~\ref{tab:transaction-rl-negative} compares both preregistered
rounds with the unchanged state-adaptive policy under the same raw stream.
R1 uses a joint-bottleneck reward and R2 changes only its aggregation to a
dense soft-min. Neither update improves the base: candidate-level strict
accuracy decreases from 17.075\% to 16.750\% and 16.775\%, while Any@20
decreases from 70.5\% to 61.5\% and 62.0\%. We therefore retain transaction
Group-RL as a negative ablation rather than part of the promoted method.

\begin{table}[t]
  \centering
  \caption{Negative transaction-branch reinforcement ablation on the same
  MolEdit gate. Both columns use the strict 0.65 source-similarity criterion,
  but candidate accuracy and condition-level Any@20 are different estimands.}
  \label{tab:transaction-rl-negative}
  \small
  \setlength{\tabcolsep}{7pt}
  \begin{tabular}{lrr}
    \toprule
    Policy & Candidate strict (\%) & Any@20 (\%) \\
    \midrule
    State-adaptive base & \textbf{17.075} & \textbf{70.5} \\
    Transaction RL R1 (joint bottleneck) & 16.750 & 61.5 \\
    Transaction RL R2 (dense soft-min) & 16.775 & 62.0 \\
    \bottomrule
  \end{tabular}
\end{table}
...
The uniform-action row reports the complete P6 candidate pool on a bounded
20-input-per-task pilot and is retained as an ablation rather than presented as
the promoted method. The state-adaptive MolProgram row is the matched P8.2
candidate-level export over 4,000 unranked candidates. We omit FCD because it is
not part of our evaluation contract.
...
The promoted state-adaptive policy achieves aggregate candidate-level
$\operatorname{Acc}_{\rm all}$ of 17.075\% at similarity 0.65 and 18.025\% at
0.15. These values remain below the corresponding MolEditRL task averages of
45.0\% and 72.7\%. At the strict threshold, MolProgram exceeds MolEditRL only
on MW$\uparrow$ (71.00\% versus 40.40\%) and on the final three-property task
Haccept$\uparrow$/MW$\uparrow$/QED$\downarrow$ (44.25\% versus 43.00\%). Its
70.5\% Any@20 instead answers whether each condition has at least one strict
hit among 20 draws and must not be compared with these candidate-level rows.
