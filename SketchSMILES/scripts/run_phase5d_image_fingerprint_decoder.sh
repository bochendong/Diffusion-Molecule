#!/usr/bin/env bash
# Run Phase 5D image-conditioned SMILES decoder with fingerprint auxiliary reranking.
...
RUN_NAME="${SKETCHSMILES_RUN_NAME:-phase5d_image_fingerprint_decoder_seed${SKETCHSMILES_SEED:-7}}"
...
FINGERPRINT_LOSS_WEIGHT="${SKETCHSMILES_FINGERPRINT_LOSS_WEIGHT:-0.25}"
RERANK_MODE="${SKETCHSMILES_RERANK_MODE:-predicted_fingerprint}"
...
echo "SketchSMILES Phase 5D image fingerprint-reranked SMILES decoder"
...
echo "  fingerprint_loss_weight=$FINGERPRINT_LOSS_WEIGHT"
...
PY
then
  echo "ERROR: Phase 5D requires NumPy, RDKit, PyTorch, and Pillow." >&2
  exit 2
fi
...
echo "[2/2] Training image-conditioned fingerprint-reranked SMILES decoder and rendering top predictions"
"$PYTHON_BIN" "${ARGS[@]}"

echo
echo "Phase 5D image fingerprint-reranked SMILES decoder finished: $OUTPUT_DIR"
