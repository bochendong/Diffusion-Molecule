#!/usr/bin/env bash
# Submit Phase 5C image-conditioned SMILES decoder to Slurm.
...
export SKETCHSMILES_RUN_NAME="${SKETCHSMILES_RUN_NAME:-phase5c_image_smiles_decoder_seed${SKETCHSMILES_SEED:-7}}"
...
echo "Submitting SketchSMILES Phase 5C image-conditioned SMILES decoder:"
...
for GPU_REQUEST in "${GPU_CANDIDATES[@]}"; do
  ...
  if sbatch \
    ...
    --export=ALL \
    --wrap="bash 'scripts/run_phase5c_image_smiles_decoder.sh'"; then
    SUBMITTED=1
    break
  fi
done
