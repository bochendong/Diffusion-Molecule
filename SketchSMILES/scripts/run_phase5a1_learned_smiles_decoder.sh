...
RANDOMIZED_SMILES_PER_MOLECULE="${SKETCHSMILES_RANDOMIZED_SMILES_PER_MOLECULE:-0}"
RANDOMIZED_SMILES_MAX_ATTEMPTS="${SKETCHSMILES_RANDOMIZED_SMILES_MAX_ATTEMPTS:-32}"
...
if [[ "$MODEL_TYPE" == "transformer" && "$RERANK_MODE" != "beam" && "$RANDOMIZED_SMILES_PER_MOLECULE" != "0" ]]; then
  PHASE_LABEL="Phase 5A-6 randomized SMILES Transformer decoder"
elif [[ "$MODEL_TYPE" == "transformer" && "$RERANK_MODE" != "beam" ]]; then
...
echo "  randomized_smiles_per_molecule=$RANDOMIZED_SMILES_PER_MOLECULE"
echo "  randomized_smiles_max_attempts=$RANDOMIZED_SMILES_MAX_ATTEMPTS"
echo "  device=$DEVICE"
...
ARGS=(
  ...
  --dropout "$DROPOUT"
  --randomized-smiles-per-molecule "$RANDOMIZED_SMILES_PER_MOLECULE"
  --randomized-smiles-max-attempts "$RANDOMIZED_SMILES_MAX_ATTEMPTS"
