...
MAX_PAIRS_PER_SOURCE="${SMMED_MAX_PAIRS_PER_SOURCE:-8}"
...
PAIRING_STRATEGY="${SMMED_PAIRING_STRATEGY:-scaffold_random}"
SOURCE_NEIGHBOR_MIN_TANIMOTO="${SMMED_SOURCE_NEIGHBOR_MIN_TANIMOTO:-0.4}"
SOURCE_NEIGHBOR_MAX_TANIMOTO="${SMMED_SOURCE_NEIGHBOR_MAX_TANIMOTO:-0.95}"
MIN_SOURCE_NEIGHBORS_T04="${SMMED_MIN_SOURCE_NEIGHBORS_T04:-1}"
...
CONDITION_CANDIDATE_DIAGNOSTICS="${SMMED_CONDITION_CANDIDATE_DIAGNOSTICS:-0}"
MIN_STRICT_CANDIDATES_T04="${SMMED_MIN_STRICT_CANDIDATES_T04:-0}"
ORACLE_FILTER_SPLITS="${SMMED_ORACLE_FILTER_SPLITS:-}"
...
echo "  pairing_strategy=$PAIRING_STRATEGY"
echo "  source_neighbor_tanimoto_range=[$SOURCE_NEIGHBOR_MIN_TANIMOTO,$SOURCE_NEIGHBOR_MAX_TANIMOTO]"
echo "  conditions_per_pair=$CONDITIONS_PER_PAIR"
echo "  condition_candidate_diagnostics=$CONDITION_CANDIDATE_DIAGNOSTICS"
echo "  oracle_filter_splits=$ORACLE_FILTER_SPLITS"
...
"$PYTHON_BIN" "$PROJECT_DIR/scripts/build_edit_pair_database.py" \
  ...
  --source-neighbor-min-tanimoto "$SOURCE_NEIGHBOR_MIN_TANIMOTO" \
  --source-neighbor-max-tanimoto "$SOURCE_NEIGHBOR_MAX_TANIMOTO" \
  --min-source-neighbors-t04 "$MIN_SOURCE_NEIGHBORS_T04" \
  --eval-fraction "$EVAL_FRACTION" \
  --seed "$SEED" \
  "${PAIR_IMAGE_ARGS[@]}"

CONDITION_ORACLE_ARGS=()
if [[ "$CONDITION_CANDIDATE_DIAGNOSTICS" == "1" ]]; then
  CONDITION_ORACLE_ARGS=(
    --candidate-molecule-db-csv "$MOLECULE_DB"
    --min-strict-candidates-t04 "$MIN_STRICT_CANDIDATES_T04"
    --oracle-filter-splits "$ORACLE_FILTER_SPLITS"
  )
fi
