#!/usr/bin/env python3
"""Refine the expanded P23 SFT adapter with normalized multi-negative margins."""
...
class PairDataset:
    def __init__(self, rows: Sequence[Mapping[str, object]], tokenizer, max_length: int):
        ...
        if not self.examples:
            raise ValueError("no P23 contrastive pairs")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    dataset = PairDataset(raw_rows, tokenizer, args.max_length)
    ...
    summary = {
        "protocol": "p23_explicit_task_contrastive_stage1_v2",
        "base_model": args.base_model, "input_adapter": str(args.input_adapter),
        "loader_kind": loader_kind, "vision_inputs_used": False,
        "logical_pair_instances": len(dataset), "negative_instances": dict(sorted(counts.items())),
        "unique_example_ids": len({row["example_id"] for row in raw_rows}),
        "epochs": args.epochs, "learning_rate": args.learning_rate,
        "chosen_ce_weight_per_unique_row": 1.0, "mode_weighting": "1:1",
