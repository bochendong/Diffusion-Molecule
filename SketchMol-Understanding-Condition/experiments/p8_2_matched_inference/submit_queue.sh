...
cpu=(--parsable --account="${P82_ACCOUNT:-rrg-hup}" --mem=32G --cpus-per-task=8 --export=ALL)
...
denovo="$(sbatch --parsable --account="${P82_ACCOUNT:-rrg-hup}" --time="${P82_DENOVO_TIME:-00:20:00}" --mem=48G --cpus-per-task=8 --gpus="${P82_GPU:-h100:1}" --export=ALL --array=2-7%6 --dependency="afterok:$pre" --job-name=p82-2p7p-s7 --output="$LOG/denovo-%A_%a.log" --wrap="bash '$HERE/run_denovo_shard.sh' \$SLURM_ARRAY_TASK_ID")"; denovo="${denovo%%;*}"
