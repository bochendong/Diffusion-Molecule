...
job=$(sbatch --parsable --account="$ACCOUNT" --job-name=p24-b5-smoke --time=00:30:00 \
  --cpus-per-task=6 --mem=64G --gres="$GRES" "${dep_args[@]}" \
  --output="$LOG_DIR/batch5-smoke-%j.log" \
  --export=ALL,P24_SCRIPT_DIR="$SCRIPT_DIR",P24_TRAIN_MODE=full,P24_OUTPUT_ROOT="$SMOKE_ROOT",P24_MAX_STEPS=20,P24_BATCH_SIZE=5,P24_GRADIENT_ACCUMULATION=13,P24_SAVE_STEPS=1000 \
