#!/usr/bin/env python3
"""Apply the preregistered P34 hard-boundary editing gate."""
...
def main() -> int:
    ...
    baseline = load_summary(args.eval_root, 0)
    candidates = {
        str(step): assess(baseline, load_summary(args.eval_root, step))
        for step in STEPS
    }
    ...
    result = {
        ...
        "decision": "PROMOTE_TO_FULL_EDIT_EVAL" if selected is not None else "STOP_P34",
        "construction_policy": "frozen and unchanged",
        "baseline_provenance": "P32.4 step-000 evaluation reused exactly",
    }
