#!/usr/bin/env bash
# Frozen B-series external transfer gate on a balanced MuMO OOD dev subset.

set -euo pipefail

STAGE="${1:?usage: run_b_series_external_mumo_transfer_v1.sh prepare|freeze|oracle|gate}"
...
RUN_ROOT="${SUCC_B_EXTERNAL_ROOT:-$PROJECT_DIR/outputs/b_series_external_mumo_transfer_v1/seed_2191}"
DATA_DIR="$PROJECT_DIR/outputs/unified_constraint_agent_mumo_parallel_evidence_v8/seed_1711/data"
BASELINE_ROOT="$PROJECT_DIR/outputs/unified_constraint_agent_mumo_closed_loop_dev_v8/seed_1711"
...
PREREG="$SCRIPT_DIR/b_series_external_mumo_transfer_v1_preregistration.json"
RUNNER="$SCRIPT_DIR/b_series_external_mumo_transfer_v1.py"
...
export PYTHONPATH="$PROJECT_CODE_DIR:$PROJECT_CODE_DIR/experiments/unified_latent_flow:$PROJECT_CODE_DIR/experiments/unified_latent_table1:$PROJECT_CODE_DIR/experiments/unified_constraint_agent:$PROJECT_CODE_DIR/scripts:$REPO_CODE_DIR/SketchMol-Unified-3MDiffusion/scripts${PYTHONPATH:+:$PYTHONPATH}"
...
case "$STAGE" in
  prepare)
    ...
    ;;
  freeze)
    "$PYTHON_BIN" "$RUNNER" freeze \
      --preregistration "$PREREG" \
      --prepare-summary "$PREPARE_DIR/prepare_summary.json" \
      --adapter-npz "$PREPARE_DIR/numeric_adapter.npz" \
      --dev-conditions "$PREPARE_DIR/generation_conditions.jsonl" \
    ...
    ;;
  oracle)
    [[ -s "$CANDIDATES" && -s "$FREEZE_DIR/freeze_summary.json" ]] || {
      echo "ERROR: frozen exact-n20 candidate artifact missing" >&2
      exit 2
    }
    ...
    "$PYTHON_BIN" "$PROJECT_DIR/scripts/evaluate_external_multiproperty_predictions.py" \
      ...
      --report-title "Frozen B-series numeric transfer MuMO balanced OOD dev exact n=20"
    ;;
  gate)
    "$PYTHON_BIN" "$RUNNER" gate \
      ...
      --baseline-detail "$BASELINE_ROOT/evaluation/external_multiproperty_detail.csv" \
