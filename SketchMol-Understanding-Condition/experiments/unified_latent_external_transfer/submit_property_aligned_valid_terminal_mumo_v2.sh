...
LOG_DIR="${SUCC_LOG_DIR:-/scratch/bdong/projects/Diffusion-Molecule/SketchMol-Understanding-Condition/logs/property_aligned_valid_terminal_mumo_v2_vocab_expanded_max64}"
RUN_SCRIPT="$SCRIPT_DIR/run_property_aligned_valid_terminal_mumo_v2.sh"
mkdir -p "$LOG_DIR"

prepare_id="$(sbatch --parsable \
  --account="$ACCOUNT_CPU" --job-name=uca-bmumo-v2m64-prepare \
  --time=01:30:00 --cpus-per-task=16 --mem=64G \
  --output="$LOG_DIR/uca-bmumo-v2m64-prepare-%j.log" \
...
printf 'summary=/scratch/bdong/projects/Diffusion-Molecule/SketchMol-Understanding-Condition/outputs/property_aligned_valid_terminal_mumo_v2_vocab_expanded_max64/seed_2211/gate/summary.json\n'
