#!/usr/bin/env python3
"""Evaluate one verifier-routed P32.1 checkpoint on the frozen dual-mode gate."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for index, record in enumerate(records):
        ...
        if (index + 1) % 10 == 0:
            print(f"[p32.1-eval:{args.tag}] {index + 1}/{len(records)}", flush=True)

    direct = mode_summary(evaluated, "direct_details")
    policy = mode_summary(evaluated, "policy_details")
    diagnostics = {}
    for mode in ("de_novo", "edit"):
        rows = [row for row in evaluated if row["task_mode"] == mode]
        failed = [row for row in rows if not row["direct_details"]["strict"]]
        diagnostics[mode] = {
            "hard_accepted": sum(bool(row["direct_details"]["strict"]) for row in rows),
            "failed_direct": len(failed),
            "strict_rescues": sum(bool(row["policy_details"]["strict"]) for row in failed),
            "first_action_kinds": dict(Counter(
                row["trace"][0]["kind"] if row["trace"] else "none" for row in rows
            )),
        }
    result = {
        "protocol": protocol.PROTOCOL,
        ...
        "target_blind": True,
        "hard_strict_direct_route": True,
        "property_reranking_within_residual_actions": False,
        "direct": direct,
        "policy": policy,
        "delta_policy_minus_direct": {
            mode: {
