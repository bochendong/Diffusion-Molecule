...
[[ -f "$OUT/SUPPORT_AUDIT_PASS" ]] || { echo "ERROR: P32.1 support audit did not pass" >&2; exit 3; }
...
"$PY" "$SCRIPT_DIR/train_residual_graph_rl.py" \
  --train-jsonl "$OUT/data/train.jsonl" \
  --base-model Qwen/Qwen2.5-1.5B-Instruct \
  --input-adapter "$P32/model/checkpoint-030/adapter" \
  --output-dir "$OUT/model" --pairs 30 --max-steps 2 \
  --max-actions 16 --site-limit 24 --score-batch-size 4 --max-length 768 \
  --temperature 0.8 --learning-rate 2e-6 --grad-clip 1.0 \
  --checkpoint-steps 10,20,30 --seed 32101
