...
"$PY" "$SCRIPT_DIR/train_multistep_trajectory_rl.py" \
  --train-jsonl "$P321/data/train.jsonl" \
  --base-model Qwen/Qwen2.5-1.5B-Instruct \
  --input-adapter "$P32/model/checkpoint-030/adapter" \
  --output-dir "$OUT/model" --pairs 30 --group-size 8 \
  --max-actions 16 --site-limit 24 --score-batch-size 4 --max-length 768 \
  --temperature 1.0 --learning-rate 2e-6 --grad-clip 1.0 \
  --checkpoint-steps 10,20,30 --seed 32201
