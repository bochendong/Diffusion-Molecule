...
for path in \
  ...
  "$P251/data/gates/final.jsonl"; do
  [[ -f "$path" ]] || { echo "ERROR: missing P32.4 input: $path" >&2; exit 2; }
done
...
"$PY" -m py_compile "$SCRIPT_DIR/source_reward.py" "$SCRIPT_DIR/train_edit_source_rloo.py" "$SCRIPT_DIR/collect_gate.py"
