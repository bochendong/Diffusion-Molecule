#!/usr/bin/env python3
"""Target-blind group-relative RL starting from the frozen P23 adapter."""
...
for path in (P23_DIR, UNIFIED_DIR):
...
def reward_response(row: Mapping[str, object], raw: str) -> tuple[float, dict[str, object]]:
    ...
    if not parsed.get("valid"):
        return -1.0, {
            "valid": False, "canonical": False, "property_strict": False,
            "strict": False, "relaxed": False, "property_fraction": 0.0,
            "source_similarity": None, "copy": False,
        }
    smiles = str(parsed["smiles"])
    score_row = scorer_row(prompt_payload(row), mode)
    ...
    if mode == "edit":
        ...
        relaxed_similarity = source_similarity >= 0.15
        strict_similarity = source_similarity >= 0.65
        copy = bool(source and unified.safe_canonical_smiles(source) == smiles)
        relaxed = bool(property_strict and relaxed_similarity)
        strict = bool(property_strict and strict_similarity)
        reward += 0.25 * min(source_similarity / 0.65, 1.0)
        reward += 0.25 * float(relaxed_similarity)
        reward += 0.50 * float(strict_similarity)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not torch.cuda.is_available() or not torch.cuda.is_bf16_supported():
        raise SystemExit("P25 requires BF16 CUDA")
    ...
    summary = {
        "protocol": "p25_p23_joint_group_rl_v1",
