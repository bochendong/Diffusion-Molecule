#!/usr/bin/env bash
# Run one bounded token-slot LoRA language-to-property representation probe.
...
PYTHON_BIN="${SUCC_TOKEN_SLOT_PYTHON:-/home/bdong/.venvs/molscribe_overlay/bin/python}"
DEP_OVERLAY="${SUCC_TOKEN_SLOT_DEP_OVERLAY:-/scratch/bdong/venvs/uca_common_llm_overlay}"
...
SFT_ADAPTER_DIR="${SUCC_TOKEN_SLOT_SFT_ADAPTER_DIR:-$SHARED_PROJECT_DIR/outputs/unified_constraint_agent_common_llm_pilot_v1/model/seed_1703/adapter}"
OUTPUT_DIR="${SUCC_TOKEN_SLOT_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/token_slot_lora_property_compiler_v3/seed_2055}"
PROTOCOL="$SCRIPT_DIR/token_slot_lora_property_compiler_v3_preregistration.json"
...
exec "$PYTHON_BIN" "$SCRIPT_DIR/token_slot_lora_property_compiler_v3.py" \
