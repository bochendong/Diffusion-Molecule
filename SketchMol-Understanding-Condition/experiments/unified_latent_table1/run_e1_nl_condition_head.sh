#!/usr/bin/env bash
# E1: frozen B41 + hashed NL projector. Two Table1 n=20 evals (template, paraphrase).
...
OUTPUT_DIR="${SUCC_E1_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/e1_nl_condition_head_table1_n20}"
...
DEVICE="${SUCC_E1_DEVICE:-auto}"
EVAL_LIMIT="${SUCC_D0_EVAL_LIMIT:-0}"
PROTOCOL="$SCRIPT_DIR/e1_nl_condition_head_preregistration.json"
...
echo "E1 NL condition head Table1 n=20"
echo "  python=$PYTHON_BIN"
echo "  device=$DEVICE"
echo "  output_dir=$OUTPUT_DIR"

"$PYTHON_BIN" "$SCRIPT_DIR/eval_e1_nl_condition_head.py" \
  --eval-csv "$EVAL_CSV" \
  ...
  --e1-protocol-manifest "$PROTOCOL" \
  --output-dir "$OUTPUT_DIR" \
  --device "$DEVICE" \
  --eval-limit "$EVAL_LIMIT"

for VARIANT in template paraphrase; do
  "$PYTHON_BIN" "$SHARED_PROJECT_DIR/scripts/evaluate_moledit_table1_anyk.py" \
    --reference "$EVAL_CSV" \
    --candidates "$OUTPUT_DIR/$VARIANT/e1_${VARIANT}_table1_n20_candidates.csv" \
    --output-dir "$OUTPUT_DIR/$VARIANT/moledit_table_metrics_any20" \
    --candidate-limit 20 \
    --model-name "e1_${VARIANT}" \
    --task-filter table1 \
  ...
  "$PYTHON_BIN" "$C_DIR/align_official_gsk3b_candidates.py" \
    ...
    --candidates "$OUTPUT_DIR/$VARIANT/e1_${VARIANT}_table1_n20_candidates.csv" \
    --output-csv "$OUTPUT_DIR/$VARIANT/e1_${VARIANT}_official_gsk3b_n20_candidates.csv"

  "$PYTHON_BIN" "$SHARED_PROJECT_DIR/scripts/evaluate_moledit_table1_anyk.py" \
    --reference "$OFFICIAL_GSK3B" \
    --candidates "$OUTPUT_DIR/$VARIANT/e1_${VARIANT}_official_gsk3b_n20_candidates.csv" \
    --output-dir "$OUTPUT_DIR/$VARIANT/official_gsk3b_metrics_any20" \
    --candidate-limit 20 \
    --model-name "e1_${VARIANT}_official_gsk3b" \
    --task-filter table1 \
    --missing-oracle-policy fail
...
"$PYTHON_BIN" "$SCRIPT_DIR/collect_e1.py" \
  --preregistration "$PROTOCOL" \
  --template-summary "$OUTPUT_DIR/template/summary.json" \
  --paraphrase-summary "$OUTPUT_DIR/paraphrase/summary.json" \
  --b41-summary "$B41_TABLE1" \
  --train-summary "$OUTPUT_DIR/train_summary.json" \
