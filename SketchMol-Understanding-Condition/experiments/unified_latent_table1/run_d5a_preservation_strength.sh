#!/usr/bin/env bash
# D5a: property-conditioned global soft eta vs constant eta on frozen B41.
# Two Table1 n=20 evals. D0 was ~40 min; two evals ≈ 90 min plus tiny head.
...
OUTPUT_DIR="${SUCC_D5A_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/d5a_preservation_strength_table1_n20}"
...
DEVICE="${SUCC_D5A_DEVICE:-auto}"
EVAL_LIMIT="${SUCC_D0_EVAL_LIMIT:-0}"
PROTOCOL="$SCRIPT_DIR/d5a_preservation_strength_preregistration.json"
...
echo "D5a preservation strength Table1 n=20"
echo "  python=$PYTHON_BIN"
echo "  device=$DEVICE"
echo "  output_dir=$OUTPUT_DIR"

"$PYTHON_BIN" "$SCRIPT_DIR/eval_d5a_preservation_strength.py" \
  --eval-csv "$EVAL_CSV" \
  ...
  --d5-protocol-manifest "$PROTOCOL" \
  --output-dir "$OUTPUT_DIR" \
  --device "$DEVICE" \
  --eval-limit "$EVAL_LIMIT"

for VARIANT in property_alpha constant_eta; do
  "$PYTHON_BIN" "$SHARED_PROJECT_DIR/scripts/evaluate_moledit_table1_anyk.py" \
    --reference "$EVAL_CSV" \
    --candidates "$OUTPUT_DIR/$VARIANT/d5a_${VARIANT}_table1_n20_candidates.csv" \
    --output-dir "$OUTPUT_DIR/$VARIANT/moledit_table_metrics_any20" \
    --candidate-limit 20 \
    --model-name "d5a_${VARIANT}" \
    --task-filter table1 \
  ...
  "$PYTHON_BIN" "$C_DIR/align_official_gsk3b_candidates.py" \
    ...
    --candidates "$OUTPUT_DIR/$VARIANT/d5a_${VARIANT}_table1_n20_candidates.csv" \
    --output-csv "$OUTPUT_DIR/$VARIANT/d5a_${VARIANT}_official_gsk3b_n20_candidates.csv"

  "$PYTHON_BIN" "$SHARED_PROJECT_DIR/scripts/evaluate_moledit_table1_anyk.py" \
    --reference "$OFFICIAL_GSK3B" \
    --candidates "$OUTPUT_DIR/$VARIANT/d5a_${VARIANT}_official_gsk3b_n20_candidates.csv" \
    --output-dir "$OUTPUT_DIR/$VARIANT/official_gsk3b_metrics_any20" \
    --candidate-limit 20 \
    --model-name "d5a_${VARIANT}_official_gsk3b" \
    --task-filter table1 \
    --missing-oracle-policy fail
...
"$PYTHON_BIN" "$SCRIPT_DIR/collect_d5a.py" \
  --preregistration "$PROTOCOL" \
  --property-summary "$OUTPUT_DIR/property_alpha/summary.json" \
  --constant-summary "$OUTPUT_DIR/constant_eta/summary.json" \
