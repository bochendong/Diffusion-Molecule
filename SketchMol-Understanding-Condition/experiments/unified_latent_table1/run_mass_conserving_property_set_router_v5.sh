#!/usr/bin/env bash
# Execute one V5 arm. Scientific acceptance is handled by a separate CPU job.
...
DEP_OVERLAY="${SUCC_SET_ROUTER_DEP_OVERLAY:-/scratch/bdong/venvs/uca_common_llm_overlay}"
...
V4_ROOT="$SHARED_PROJECT_DIR/outputs/structured_sparse_property_router_v4/seed_2061"
...
SFT_ADAPTER_DIR="${SUCC_SET_ROUTER_SFT_ADAPTER_DIR:-$SHARED_PROJECT_DIR/outputs/unified_constraint_agent_common_llm_pilot_v1/model/seed_1703/adapter}"
OUTPUT_DIR="${SUCC_SET_ROUTER_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/mass_conserving_property_set_router_v5/seed_2071}"
PROTOCOL="$SCRIPT_DIR/mass_conserving_property_set_router_v5_preregistration.json"
V4_MANIFEST="$SCRIPT_DIR/structured_sparse_property_router_v4_preregistration.json"
E1_MANIFEST="$SCRIPT_DIR/e1_nl_condition_head_preregistration.json"
ARMS=(full no_lora no_token_slots no_composition)
ARM_INDEX="${SLURM_ARRAY_TASK_ID:-${SUCC_SET_ROUTER_ARM_INDEX:-0}}"

if (( ARM_INDEX < 0 || ARM_INDEX >= ${#ARMS[@]} )); then
  echo "Invalid V5 arm index: $ARM_INDEX" >&2
  exit 2
fi
ARM="${ARMS[$ARM_INDEX]}"
...
exec "$PYTHON_BIN" "$SCRIPT_DIR/mass_conserving_property_set_router_v5.py" \
  ...
  --v4-gate-summary "$V4_ROOT/gate/gate_summary.json" \
