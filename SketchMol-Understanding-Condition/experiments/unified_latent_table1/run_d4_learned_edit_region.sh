#!/usr/bin/env bash
# D4b: learned property-conditioned edit region vs matched random vs shuffled.
# One process trains the head then samples three variants on frozen B41.
...
OUTPUT_DIR="${SUCC_D4B_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/d4_learned_edit_region_table1_n20}"
...
for path in \
  ...
  "$SCRIPT_DIR/d4_learned_edit_region_preregistration.json"; do
...
echo "D4b learned edit region Table1 n=20"
echo "  python=$PYTHON_BIN"
echo "  device=$DEVICE"
echo "  output_dir=$OUTPUT_DIR"

"$PYTHON_BIN" "$SCRIPT_DIR/eval_d4_learned_edit_region.py" \
  --eval-csv "$EVAL_CSV" \
  ...
  --d4-protocol-manifest "$SCRIPT_DIR/d4_learned_edit_region_preregistration.json" \
  --output-dir "$OUTPUT_DIR" \
  --device "$DEVICE" \
  --eval-limit "$EVAL_LIMIT"

for VARIANT in learned_hard random_matched shuffled_learned; do
  "$PYTHON_BIN" "$SHARED_PROJECT_DIR/scripts/evaluate_moledit_table1_anyk.py" \
    --reference "$EVAL_CSV" \
    --candidates "$OUTPUT_DIR/$VARIANT/d4_${VARIANT}_table1_n20_candidates.csv" \
    --output-dir "$OUTPUT_DIR/$VARIANT/moledit_table_metrics_any20" \
    --candidate-limit 20 \
    --model-name "d4_${VARIANT}" \
    --task-filter table1 \
  ...
  "$PYTHON_BIN" "$C_DIR/align_official_gsk3b_candidates.py" \
    ...
    --candidates "$OUTPUT_DIR/$VARIANT/d4_${VARIANT}_table1_n20_candidates.csv" \
    --output-csv "$OUTPUT_DIR/$VARIANT/d4_${VARIANT}_official_gsk3b_n20_candidates.csv"

  "$PYTHON_BIN" "$SHARED_PROJECT_DIR/scripts/evaluate_moledit_table1_anyk.py" \
    --reference "$OFFICIAL_GSK3B" \
    --candidates "$OUTPUT_DIR/$VARIANT/d4_${VARIANT}_official_gsk3b_n20_candidates.csv" \
    --output-dir "$OUTPUT_DIR/$VARIANT/official_gsk3b_metrics_any20" \
    --candidate-limit 20 \
    --model-name "d4_${VARIANT}_official_gsk3b" \
    --task-filter table1 \
    --missing-oracle-policy fail
  ...
  "$PYTHON_BIN" "$C_DIR/collect_c1_table1_n20.py" \
    ...
    --preregistration "$SCRIPT_DIR/d4_learned_edit_region_preregistration.json" \
...
"$PYTHON_BIN" "$SCRIPT_DIR/collect_d4_learned.py" \
  --preregistration "$SCRIPT_DIR/d4_learned_edit_region_preregistration.json" \
  --learned-summary "$OUTPUT_DIR/learned_hard/summary.json" \
  --random-summary "$OUTPUT_DIR/random_matched/summary.json" \
  --shuffled-summary "$OUTPUT_DIR/shuffled_learned/summary.json" \
  --b41-summary "$SHARED_PROJECT_DIR/outputs/d0_b41_table1_n20/summary.json" \
