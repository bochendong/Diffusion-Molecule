#!/usr/bin/env bash
# Execute the target-free V10 representation gate or its separate science gate.
...
PYTHON_BIN="${SUCC_V10_PYTHON:-/home/bdong/.venvs/molscribe_overlay/bin/python}"
DEP_OVERLAY="${SUCC_V10_DEP_OVERLAY:-/scratch/bdong/venvs/uca_common_llm_overlay}"
STAGE="${SUCC_V10_STAGE:?Set SUCC_V10_STAGE to execute or gate}"
OUTPUT_ROOT="${SUCC_V10_OUTPUT_ROOT:-$SHARED_PROJECT_DIR/outputs/permutation_invariant_semantic_set_compiler_v10/seed_2151}"
...
PROTOCOL="$SCRIPT_DIR/permutation_invariant_semantic_set_compiler_v10_preregistration.json"
...
case "$STAGE" in
  execute)
    exec "$PYTHON_BIN" "$SCRIPT_DIR/permutation_invariant_semantic_set_compiler_v10.py" \
    ...
    ;;
  gate)
    exec "$PYTHON_BIN" "$SCRIPT_DIR/gate_permutation_invariant_semantic_set_compiler_v10.py" \
      --protocol-manifest "$PROTOCOL" \
      --execution-summary "$EXECUTION_DIR/summary.json" \
      --output-dir "$GATE_DIR"
    ;;
  *)
    echo "ERROR: unsupported V10 stage: $STAGE" >&2
    exit 2
    ;;
esac
