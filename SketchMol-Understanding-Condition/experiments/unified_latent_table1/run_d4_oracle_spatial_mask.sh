#!/usr/bin/env bash
# D4a: oracle hard spatial mask vs matched random, frozen B41 Table1 n=20.
...
OUTPUT_DIR="${SUCC_D4_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/d4_oracle_spatial_mask_table1_n20}"
...
for path in \
  ...
  "$SCRIPT_DIR/d4_oracle_spatial_mask_preregistration.json"; do
...
COMMON_ARGS=(
  ...
  --d4-protocol-manifest "$SCRIPT_DIR/d4_oracle_spatial_mask_preregistration.json"
  --output-dir "$OUTPUT_DIR"
  --device "$DEVICE"
  --eval-limit "$EVAL_LIMIT"
)

for VARIANT in oracle_hard random_matched; do
  echo "D4a $VARIANT frozen B41 Table1 n=20"
  echo "  python=$PYTHON_BIN"
  echo "  device=$DEVICE"
  echo "  output_dir=$OUTPUT_DIR/$VARIANT"

  "$PYTHON_BIN" "$SCRIPT_DIR/eval_d4_oracle_spatial_mask.py" "${COMMON_ARGS[@]}" --variant "$VARIANT"
  ...
  "$PYTHON_BIN" "$C_DIR/collect_c1_table1_n20.py" \
    ...
    --preregistration "$SCRIPT_DIR/d4_oracle_spatial_mask_preregistration.json" \
...
"$PYTHON_BIN" "$SCRIPT_DIR/collect_d4_headroom.py" \
  --preregistration "$SCRIPT_DIR/d4_oracle_spatial_mask_preregistration.json" \
  --oracle-summary "$OUTPUT_DIR/oracle_hard/summary.json" \
