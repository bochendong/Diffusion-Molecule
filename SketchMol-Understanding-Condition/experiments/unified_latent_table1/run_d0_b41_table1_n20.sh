#!/usr/bin/env bash
# D0b: frozen B41 + valid-terminal STOP on Table1 n=20.
...
OUTPUT_DIR="${SUCC_D0_B41_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/d0_b41_table1_n20}"
...
DEVICE="${SUCC_D0_B41_DEVICE:-auto}"
...
for path in \
  ...
  "$SCRIPT_DIR/d0_b41_preregistration.json"; do
  [[ -f "$path" ]] || { echo "ERROR: missing input: $path" >&2; exit 2; }
done

mkdir -p "$OUTPUT_DIR"
export PYTHONPATH="$WORKTREE_LATENT:$WORKTREE_LATENT/../..:$WORKTREE_LATENT/../../experiments/unified_constraint_agent:$PROJECT_DIR:$C_DIR:$REPO_DIR/SketchMol-Unified-3MDiffusion/scripts${PYTHONPATH:+:$PYTHONPATH}"
...
echo "D0b B41 valid-terminal Table1 n=20"
echo "  python=$PYTHON_BIN"
echo "  device=$DEVICE"
echo "  output_dir=$OUTPUT_DIR"

"$PYTHON_BIN" "$SCRIPT_DIR/eval_d0_b41_table1.py" \
  --eval-csv "$EVAL_CSV" \
  ...
  --d0-protocol-manifest "$SCRIPT_DIR/d0_b41_preregistration.json" \
...
"$PYTHON_BIN" "$SHARED_PROJECT_DIR/scripts/evaluate_moledit_table1_anyk.py" \
  --reference "$EVAL_CSV" \
  --candidates "$OUTPUT_DIR/d0_b41_table1_n20_candidates.csv" \
  --output-dir "$OUTPUT_DIR/moledit_table_metrics_any20" \
  --candidate-limit 20 \
  --model-name d0_b41_valid_terminal \
  --task-filter table1 \
...
"$PYTHON_BIN" "$C_DIR/align_official_gsk3b_candidates.py" \
  ...
  --candidates "$OUTPUT_DIR/d0_b41_table1_n20_candidates.csv" \
  --output-csv "$OUTPUT_DIR/d0_b41_official_gsk3b_n20_candidates.csv"

"$PYTHON_BIN" "$SHARED_PROJECT_DIR/scripts/evaluate_moledit_table1_anyk.py" \
  --reference "$OFFICIAL_GSK3B" \
  --candidates "$OUTPUT_DIR/d0_b41_official_gsk3b_n20_candidates.csv" \
  --output-dir "$OUTPUT_DIR/official_gsk3b_metrics_any20" \
  --candidate-limit 20 \
  --model-name d0_b41_valid_terminal_official_gsk3b \
  --task-filter table1 \
  --missing-oracle-policy fail
...
"$PYTHON_BIN" "$C_DIR/collect_c1_table1_n20.py" \
  ...
  --preregistration "$SCRIPT_DIR/d0_b41_preregistration.json" \
