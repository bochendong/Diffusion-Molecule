...
FROZEN_MODEL_CHECKPOINT="${SUCC_D3_FROZEN_MODEL_CHECKPOINT:-}"
GENERATE_ONLY="${SUCC_D3_GENERATE_ONLY:-0}"
...
for path in \
  ...
  "$LATENT_DIR/viability_preserving_interacting_particle_transport_v41_preregistration.json" \
  "$PROTOCOL"; do
  [[ -f "$path" ]] || { echo "ERROR: missing input: $path" >&2; exit 2; }
done

mkdir -p "$OUTPUT_DIR"
export PYTHONPATH="$LATENT_DIR:$PROJECT_DIR:$C_DIR:$SCRIPT_DIR:$REPO_DIR/SketchMol-Unified-3MDiffusion/scripts${PYTHONPATH:+:$PYTHONPATH}"
...
echo "  frozen_model_checkpoint=${FROZEN_MODEL_CHECKPOINT:-none}"

D3_EXTRA_ARGS=()
if [[ -n "$FROZEN_MODEL_CHECKPOINT" ]]; then
  [[ -f "$FROZEN_MODEL_CHECKPOINT" ]] || { echo "ERROR: missing frozen D3 checkpoint: $FROZEN_MODEL_CHECKPOINT" >&2; exit 2; }
  D3_EXTRA_ARGS+=(--frozen-model-checkpoint "$FROZEN_MODEL_CHECKPOINT")
fi
...
"$PYTHON_BIN" "$SCRIPT_DIR/eval_d3_event_kernel_energy.py" \
  ...
  --b41-protocol-manifest "$LATENT_DIR/viability_preserving_interacting_particle_transport_v41_preregistration.json" \
...
if [[ "$GENERATE_ONLY" == "1" ]]; then
  echo "D3 frozen teacher candidate generation ready: $OUTPUT_DIR/d3_event_kernel_energy_table1_n20_candidates.csv"
  exit 0
fi
