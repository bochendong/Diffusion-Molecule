...
OUTPUT_ROOT="${SUCC_V7_OUTPUT_ROOT:-$SHARED_REPO_DIR/SketchMol-Understanding-Condition/outputs/mass_conserving_router_fresh_horizon_v7/seed_2093}"
...
RESUME_AFTER_PREPARE="${SUCC_V7_RESUME_AFTER_PREPARE:-0}"
mkdir -p "$LOG_DIR"

freeze_dependency=()
if [[ "$RESUME_AFTER_PREPARE" == "1" ]]; then
  test -s "$OUTPUT_ROOT/prepare/summary.json"
  prepare_job="completed_prepare_artifact"
else
  ...
  freeze_dependency=(--dependency="afterok:$prepare_job" --kill-on-invalid-dep=yes)
fi
...
echo "resume_after_prepare=$RESUME_AFTER_PREPARE"
