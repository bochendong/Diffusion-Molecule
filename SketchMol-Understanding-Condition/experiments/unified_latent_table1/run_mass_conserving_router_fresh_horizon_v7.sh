#!/usr/bin/env bash
# Run one physically isolated V7 stage.
...
PYTHON_BIN="${SUCC_V7_PYTHON:-/home/bdong/.venvs/molscribe_overlay/bin/python}"
DEP_OVERLAY="${SUCC_V7_DEP_OVERLAY:-/scratch/bdong/venvs/uca_common_llm_overlay}"
STAGE="${SUCC_V7_STAGE:?Set SUCC_V7_STAGE}"
OUTPUT_ROOT="${SUCC_V7_OUTPUT_ROOT:-$SHARED_PROJECT_DIR/outputs/mass_conserving_router_fresh_horizon_v7/seed_2093}"
...
PROTOCOL="$SCRIPT_DIR/mass_conserving_router_fresh_horizon_v7_preregistration.json"
...
COMMON=("$PYTHON_BIN" "$SCRIPT_DIR/mass_conserving_router_fresh_horizon_v7.py" --protocol-manifest "$PROTOCOL")
...
case "$STAGE" in
  prepare)
    exec "${COMMON[@]}" prepare \
      ...
      --v6-gate "$V6_ROOT/gate/gate_summary.json" \
  ...
  gate)
    ...
    ;;
  *)
    echo "ERROR: unsupported V7 stage: $STAGE" >&2
    exit 2
    ;;
esac
