...
for path in (WORKTREE_LATENT, WORKTREE_PROJECT, C_DIR, PROJECT_DIR, UNIFIED_SCRIPTS, SCRIPT_DIR):
...
def main() -> int:
    ...
    supervised_path = args.output_dir / "d3_event_kernel_energy_supervised.pt"
    if bool(d3.get("resume_supervised")) and supervised_path.exists():
    ...
    skip_grpo = bool(d3.get("skip_grpo")) or int(d3.get("grpo_epochs", 0)) <= 0
    grpo_conditions: list = []
    grpo_history: list[dict[str, float]] = []
    if not skip_grpo:
        chem = Chemistry()
    ...
    try:
        if skip_grpo:
            print(
                json.dumps(
                    {
                        "stage": "grpo_skipped",
                        "reason": "10GB cannot hold GRPO; evaluate supervised checkpoint only",
                    },
                    sort_keys=True,
                ),
                flush=True,
        ...
        checkpoint_path = args.output_dir / "d3_event_kernel_energy.pt"
        torch.save(
            {
                "model_state": model.state_dict(),
                "model_config": b41_checkpoint.get("model_config"),
