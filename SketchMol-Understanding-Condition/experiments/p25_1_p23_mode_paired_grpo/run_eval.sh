...
METHOD="${P251_EVAL_METHOD:?P251_EVAL_METHOD must be baseline or rl}"
case "$SPLIT" in dev|final) ;; *) echo "ERROR: unsupported gate split $SPLIT" >&2; exit 2 ;; esac
case "$METHOD" in
  baseline) ADAPTER="$P23/model/stage1_v2/adapter"; REQUIRED="$OUT/PREPARED" ;;
  rl) ADAPTER="$OUT/model/p25_1/adapter"; REQUIRED="$OUT/TRAIN_COMPLETE" ;;
...
for path in "$REQUIRED" "$OUT/data/gates/$SPLIT.jsonl" "$ADAPTER/adapter_model.safetensors" \
  "$SUCC_GSK3B_ORACLE_PATH" "$SUCC_DRD2_ORACLE_PATH"; do
  [[ -f "$path" ]] || { echo "ERROR: missing P25.1 eval input: $path" >&2; exit 2; }
done
...
"$PY" "$PROJECT/experiments/p25_p23_joint_group_rl/evaluate_frozen_gate.py" \
  --gate-jsonl "$OUT/data/gates/$SPLIT.jsonl" --base-model "$BASE" \
  --adapter-dir "$ADAPTER" --output-dir "$OUT/gate/$SPLIT/$METHOD" \
  --method "$METHOD" --repeats 4 --batch-size 8 --seed 251251
touch "$OUT/GATE_${SPLIT}_${METHOD}_COMPLETE"
