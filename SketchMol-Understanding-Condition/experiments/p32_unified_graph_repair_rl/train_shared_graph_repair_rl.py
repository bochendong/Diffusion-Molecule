#!/usr/bin/env python3
"""Train one shared LoRA with paired exact GraphEditDSL policy gradients."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    by_mode = {
        mode: [row for row in rows if row["task_mode"] == mode]
        for mode in ("de_novo", "edit")
    }
    if any(len(values) < args.pairs for values in by_mode.values()):
        raise ValueError({mode: len(values) for mode, values in by_mode.items()})
    for offset, mode in enumerate(("de_novo", "edit")):
        random.Random(args.seed + offset).shuffle(by_mode[mode])
        by_mode[mode] = by_mode[mode][: args.pairs]
    ...
    for pair_index, (denovo, edit) in enumerate(
        ...
        for offset, mode in enumerate(("de_novo", "edit")):
            supports, final_smiles = protocol.online_supports(
                model, tokenizer, records[mode],
                max_steps=args.max_steps, max_actions=args.max_actions,
                site_limit=args.site_limit, max_length=args.max_length,
                score_batch_size=args.score_batch_size,
                temperature=args.temperature,
                seed=args.seed + pair_index * 1009 + offset,
            )
            if not supports:
                raise RuntimeError(f"no executable P32 support for {mode} pair {pair_index}")
        ...
        for mode in ("de_novo", "edit"):
            ...
            loss, action_count, exact = common_rl.exact_action_value_backward(
                model, tokenizer, supports_by_mode[mode],
                args=helper_args, gradient_divisor=1,
            )
        ...
        if not finite:
            raise FloatingPointError(f"non-finite P32 merged gradient at pair {pair_index}")
        ...
        if nonfinite:
            raise FloatingPointError(f"P32 adapter has {nonfinite} non-finite values")
        record = {
