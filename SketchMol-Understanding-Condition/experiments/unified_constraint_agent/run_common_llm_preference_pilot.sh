#!/usr/bin/env bash
# Build train-only action preferences, tune seed_1703, and compare rankings.
...
PREF_ROOT="${SUCC_UCA_PREFERENCE_ROOT:-$PROJECT_DIR/outputs/unified_constraint_agent_preference_v1}"
INPUT_SEED="${SUCC_UCA_INPUT_SEED:-1703}"
OUTPUT_SEED="${SUCC_UCA_SEED:-1704}"
...
for path in "$SFT_DATA/train.jsonl" "$SFT_DATA/validation.jsonl" "$INPUT_ADAPTER/adapter_model.safetensors"; do
  [[ -f "$path" ]] || { echo "ERROR: missing preference pilot input: $path" >&2; exit 2; }
done

if [[ "${SUCC_UCA_SKIP_PREFERENCE_DATA:-0}" != "1" ]]; then
  "$PYTHON_BIN" "$SCRIPT_DIR/build_common_llm_action_preferences.py" \
...
if [[ "${SUCC_UCA_SKIP_PREFERENCE_TRAIN:-0}" != "1" ]]; then
  "$PYTHON_BIN" "$SCRIPT_DIR/train_common_llm_preference.py" \
    ...
    --learning-rate "${SUCC_UCA_PREFERENCE_LR:-2e-5}" \
    --beta "${SUCC_UCA_PREFERENCE_BETA:-5.0}" \
    --sft-weight "${SUCC_UCA_PREFERENCE_SFT_WEIGHT:-0.05}" \
    --seed "$OUTPUT_SEED"
fi
...
"$PYTHON_BIN" "$SCRIPT_DIR/evaluate_common_llm_constrained_actions.py" \
  "${common_eval_args[@]}" \
  --output-dir "$EVAL_DIR/preference" \
  --adapter-dir "$MODEL_DIR/adapter" \
  --variant preference

echo "Common-LLM preference pilot ready: $PREF_ROOT"
