#!/usr/bin/env bash
# Train and gate the paired Table1/MuMO PCGrad common-LLM tool policy.
...
RUN_ROOT="${SUCC_UCA_TOOL_POLICY_PCGRAD_ROOT:-$PROJECT_DIR/outputs/unified_constraint_agent_tool_policy_pcgrad_v3/seed_${SEED}}"
...
for path in \
  ...
  "$GSK3B_ORACLE"; do
  [[ -e "$path" ]] || { echo "ERROR: missing PCGrad-policy input: $path" >&2; exit 2; }
done
...
echo "=== Train paired-PCGrad exact-action-value common-LLM tool policy ==="
"$PYTHON_BIN" "$SCRIPT_DIR/train_common_llm_tool_policy_grpo.py" \
  ...
  --policy-update paired_pcgrad_exact_action_value \
  --rollout-origins table1,mumo \
...
echo "Paired-PCGrad tool policy ready: $RUN_ROOT"
