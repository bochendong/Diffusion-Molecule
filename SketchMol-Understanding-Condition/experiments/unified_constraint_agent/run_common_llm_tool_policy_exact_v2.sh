#!/usr/bin/env bash
# Train and gate the exact-action-value common-LLM tool policy.
...
SEED="${SUCC_UCA_EXACT_SEED:-1708}"
RUN_ROOT="${SUCC_UCA_TOOL_POLICY_EXACT_ROOT:-$PROJECT_DIR/outputs/unified_constraint_agent_tool_policy_exact_v2/seed_${SEED}}"
...
for path in \
  ...
  "$GSK3B_ORACLE"; do
  [[ -e "$path" ]] || { echo "ERROR: missing exact-policy input: $path" >&2; exit 2; }
done
...
echo "=== Train exact-action-value common-LLM tool policy ==="
"$PYTHON_BIN" "$SCRIPT_DIR/train_common_llm_tool_policy_grpo.py" \
  ...
  --policy-update exact_action_value \
  --rollout-origins "${SUCC_UCA_ROLLOUT_ORIGINS:-table1,mumo}" \
  --max-train-per-origin "${SUCC_UCA_MAX_TRAIN_PER_ORIGIN:-16}" \
  --max-validation-per-origin "${SUCC_UCA_MAX_VALIDATION_PER_ORIGIN:-8}" \
  --paths-per-condition "${SUCC_UCA_PATHS_PER_CONDITION:-1}" \
  --validation-rollouts "${SUCC_UCA_VALIDATION_ROLLOUTS:-8}" \
  ...
  --gradient-accumulation "${SUCC_UCA_GRADIENT_ACCUMULATION:-2}" \
  --learning-rate "${SUCC_UCA_LR:-1e-5}" \
  --anchor-sft-weight "${SUCC_UCA_ANCHOR_SFT_WEIGHT:-0.10}" \
  --anchor-max-per-origin "${SUCC_UCA_ANCHOR_MAX_PER_ORIGIN:-64}" \
  --retention-max-per-origin "${SUCC_UCA_RETENTION_MAX_PER_ORIGIN:-24}" \
  --retention-max-regression "${SUCC_UCA_RETENTION_MAX_REGRESSION:-0.05}" \
  --seed "$SEED"

echo "Exact-action-value tool policy ready: $RUN_ROOT"
