...
TAG="${P301_EVAL_TAG:-rl}"
ADAPTER="${P301_EVAL_ADAPTER:-$OUT/model/balanced_shared_rl/adapter}"
...
"$PY" "$SCRIPT_DIR/evaluate_small_raw1_gate.py" \
  ...
  --output-dir "$OUT/small_gate/$TAG" --batch-size 8 \
  --macro-delta-min 0.02 --valid-delta-min -0.01 --bucket-delta-min -0.10
touch "$OUT/SMALL_GATE_${TAG}_COMPLETE"
