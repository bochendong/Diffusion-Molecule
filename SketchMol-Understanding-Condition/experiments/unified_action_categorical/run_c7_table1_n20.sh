#!/usr/bin/env bash
# C7: C5 warm-start GRPO on real Table1 train rows, then honest n=20 mixture.
...
OUTPUT_DIR="${SUCC_C7_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/joint_graph_fragment_categorical_c7}"
EVAL_CSV="${SUCC_C7_EVAL_CSV:-$SHARED_PROJECT_DIR/outputs/unified_smiles_generator_joint_v2/dataset/table1_test_rows.csv}"
TRAIN_MOLEDIT="${SUCC_C7_TRAIN_MOLEDIT_CSV:-$SHARED_PROJECT_DIR/outputs/unified_smiles_generator_suite_v1/dataset/table1_train_pack/table1_moledit_rows.csv}"
TRAIN_CONDITION="${SUCC_C7_TRAIN_CONDITION_CSV:-$SHARED_PROJECT_DIR/outputs/unified_smiles_generator_suite_v1/dataset/table1_train_pack/table1_benchmark_condition_rows.csv}"
TRAIN_CSV="${SUCC_C7_TRAIN_CSV:-$OUTPUT_DIR/dataset/table1_train_real_rows.csv}"
GRAPH_CANDIDATES="${SUCC_C7_GRAPH_CANDIDATES:-$SHARED_PROJECT_DIR/outputs/umtp_graph_action_full_eval_v1/seed_7/eval/action/table1/candidate_pool/graph_action_candidates.csv}"
OFFICIAL_GSK3B="${SUCC_C7_OFFICIAL_GSK3B_CSV:-$SHARED_PROJECT_DIR/outputs/direct_smiles_moledit_table1_gsk3b_n20_pilot_v1/gsk3b_pack/table1_eval_gsk3b_moledit_rows.csv}"
BASE_CHECKPOINT="${SUCC_C7_BASE_CHECKPOINT:-$SHARED_PROJECT_DIR/outputs/joint_graph_fragment_categorical_c5/umtp_graph_action_policy_grpo.pt}"
TRAIN_FEATURES="${SUCC_C7_TRAIN_FEATURES:-$SHARED_PROJECT_DIR/outputs/unified_smiles_generator_suite_v1/feature_variants/train_condition_features_hf_vlm}"
EVAL_FEATURES="${SUCC_C7_EVAL_FEATURES:-$SHARED_PROJECT_DIR/outputs/unified_smiles_generator_suite_v1/feature_variants/eval_condition_features_hf_vlm}"
...
TRAIN_DEVICE="${SUCC_C7_TRAIN_DEVICE:-auto}"
EVAL_DEVICE="${SUCC_DEVICE:-cpu}"
EVAL_LIMIT="${SUCC_C7_EVAL_LIMIT:-0}"
TRAIN_LIMIT="${SUCC_C7_TRAIN_LIMIT:-0}"
SCORE_BATCH="${SUCC_C7_SCORE_BATCH:-64}"
...
for path in \
  ...
  "$SCRIPT_DIR/joint_graph_fragment_categorical_c7_preregistration.json"; do
...
echo "C7 real-train graph GRPO + C1 mixture n=20"
...
"$PYTHON_BIN" "$SCRIPT_DIR/train_graph_action_grpo_c5.py" \
  --c5-protocol-manifest "$SCRIPT_DIR/joint_graph_fragment_categorical_c7_preregistration.json" \
...
"$PYTHON_BIN" "$SCRIPT_DIR/eval_joint_graph_fragment_categorical_c1.py" \
  ...
  --c1-protocol-manifest "$SCRIPT_DIR/joint_graph_fragment_categorical_c7_preregistration.json" \
  --output-dir "$OUTPUT_DIR" \
  --candidate-output "$OUTPUT_DIR/c7_table1_n20_candidates.csv" \
  --device "$EVAL_DEVICE" \
  --eval-limit "$EVAL_LIMIT"

"$PYTHON_BIN" "$SHARED_PROJECT_DIR/scripts/evaluate_moledit_table1_anyk.py" \
  --reference "$EVAL_CSV" \
  --candidates "$OUTPUT_DIR/c7_table1_n20_candidates.csv" \
  --output-dir "$OUTPUT_DIR/moledit_table_metrics_any20" \
  --candidate-limit 20 \
  --model-name joint_graph_fragment_categorical_c7 \
  --task-filter table1 \
...
"$PYTHON_BIN" "$SCRIPT_DIR/align_official_gsk3b_candidates.py" \
  ...
  --candidates "$OUTPUT_DIR/c7_table1_n20_candidates.csv" \
  --output-csv "$OUTPUT_DIR/c7_official_gsk3b_n20_candidates.csv"

"$PYTHON_BIN" "$SHARED_PROJECT_DIR/scripts/evaluate_moledit_table1_anyk.py" \
  --reference "$OFFICIAL_GSK3B" \
  --candidates "$OUTPUT_DIR/c7_official_gsk3b_n20_candidates.csv" \
  --output-dir "$OUTPUT_DIR/official_gsk3b_metrics_any20" \
  --candidate-limit 20 \
  --model-name joint_graph_fragment_categorical_c7_official_gsk3b \
  --task-filter table1 \
  --missing-oracle-policy fail
...
"$PYTHON_BIN" "$SCRIPT_DIR/collect_c1_table1_n20.py" \
  ...
  --preregistration "$SCRIPT_DIR/joint_graph_fragment_categorical_c7_preregistration.json" \
