#!/usr/bin/env bash
# Generate one frozen n=256 D4 candidate pool without retraining.
...
VARIANT="${D4_VARIANT:-${1:-}}"
SEED="${SUCC_D4_SEED:-7}"
NUM_SAMPLES="${SUCC_D4_NUM_SAMPLES:-256}"
OUTPUT_ROOT="${SUCC_D4_OUTPUT_ROOT:-$PROJECT_DIR/outputs/d4_property_program_group_rl_seed7}"

if [[ "$NUM_SAMPLES" != "256" ]]; then
  echo "ERROR: D4 preregistration fixes num_samples=256; got $NUM_SAMPLES" >&2
  exit 2
fi
...
case "$VARIANT" in
  ...
  ood_group_rl)
    ...
    ;;
  *)
    echo "Usage: D4_VARIANT={two_p_to_seven_p_sft|two_p_to_seven_p_group_rl|ood_sft|ood_group_rl} $0" >&2
    exit 2
    ;;
esac
...
for required in "$CHECKPOINT" "$EVAL_CSV" "$TRAIN_FEATURES" "$EVAL_FEATURES"; do
  if [[ ! -e "$required" ]]; then
    echo "ERROR: missing D4 input: $required" >&2
    exit 2
  fi
done

if [[ -f "$COMPLETE_MARKER" && -s "$CANDIDATE_CSV" ]]; then
  echo "D4 candidate pool already complete; reusing $CANDIDATE_CSV"
  exit 0
fi

echo "D4 candidate generation"
...
echo "  note=diagnostic selected CSV is property-reranked and excluded from D4 metrics"
...
"$PYTHON_BIN" "$PROJECT_DIR/scripts/train_direct_smiles_generator.py" \
  ...
  --eval-batch-size "${SUCC_D4_EVAL_BATCH_SIZE:-32}" \
  --max-new-tokens "${SUCC_D4_MAX_NEW_TOKENS:-96}" \
  --temperature "$TEMPERATURE" \
  --top-k "$TOP_K" \
  --top-p "$TOP_P" \
  --num-samples "$NUM_SAMPLES" \
  --parallel-samples "${SUCC_D4_PARALLEL_SAMPLES:-8}" \
  --max-parallel-sequences "${SUCC_D4_MAX_PARALLEL_SEQUENCES:-1024}" \
  --repetition-penalty "$REPETITION_PENALTY" \
  --no-repeat-ngram-size "${SUCC_D4_NO_REPEAT_NGRAM_SIZE:-6}" \
  --min-new-tokens "${SUCC_D4_MIN_NEW_TOKENS:-6}" \
  --seed "$SEED" \
  --device "${SUCC_D4_DEVICE:-auto}"

cp "$SCRIPT_DIR/d4_property_program_group_rl_preregistration.json" "$OUTPUT_DIR/protocol_snapshot.json"
touch "$COMPLETE_MARKER"
echo "D4 variant complete: $VARIANT"
