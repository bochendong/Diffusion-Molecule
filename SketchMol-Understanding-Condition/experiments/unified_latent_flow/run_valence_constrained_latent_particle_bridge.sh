#!/usr/bin/env bash
# B40: frozen B39 with dynamic train-only valence support and exact 20 particles.
...
PYTHON_BIN="${SUCC_VALENCE_PARTICLE_PYTHON:-/home/bdong/.venvs/molscribe_overlay/bin/python}"
SEED="${SUCC_VALENCE_PARTICLE_SEED:-1989}"
OUTPUT_DIR="${SUCC_VALENCE_PARTICLE_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/valence_constrained_latent_particle_bridge_v40/seed_${SEED}}"
...
PREREGISTRATION="$SCRIPT_DIR/valence_constrained_latent_particle_bridge_v40_preregistration.json"
...
for path in \
  ...
  "$PREREGISTRATION"; do
  [[ -f "$path" ]] || { echo "ERROR: missing B40 input: $path" >&2; exit 2; }
done

if [[ -f "$OUTPUT_DIR/summary.json" && "${SUCC_VALENCE_PARTICLE_FORCE:-0}" != "1" ]]; then
  echo "ERROR: completed B40 result exists: $OUTPUT_DIR/summary.json" >&2
  exit 2
fi
...
exec "$PYTHON_BIN" "$SCRIPT_DIR/valence_constrained_latent_particle_bridge.py" \
