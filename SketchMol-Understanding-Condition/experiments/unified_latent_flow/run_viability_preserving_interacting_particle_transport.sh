#!/usr/bin/env bash
# B41: support-consistent event fine-tuning plus exact-20 interacting particles.
...
PYTHON_BIN="${SUCC_VIABILITY_PARTICLE_PYTHON:-/home/bdong/.venvs/molscribe_overlay/bin/python}"
SEED="${SUCC_VIABILITY_PARTICLE_SEED:-1991}"
OUTPUT_DIR="${SUCC_VIABILITY_PARTICLE_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/viability_preserving_interacting_particle_transport_v41/seed_${SEED}}"
...
PREREGISTRATION="$SCRIPT_DIR/viability_preserving_interacting_particle_transport_v41_preregistration.json"
...
for path in \
  ...
  "$PREREGISTRATION"; do
  [[ -f "$path" ]] || { echo "ERROR: missing B41 input: $path" >&2; exit 2; }
done

if [[ -f "$OUTPUT_DIR/summary.json" && "${SUCC_VIABILITY_PARTICLE_FORCE:-0}" != "1" ]]; then
  echo "ERROR: completed B41 result exists: $OUTPUT_DIR/summary.json" >&2
  exit 2
fi
...
exec "$PYTHON_BIN" "$SCRIPT_DIR/viability_preserving_interacting_particle_transport.py" \
