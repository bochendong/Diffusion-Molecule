#!/usr/bin/env bash
# B38: orderless one-event latent graph jump process, exact n=20.
...
PYTHON_BIN="${SUCC_GRAPH_JUMP_PYTHON:-/home/bdong/.venvs/molscribe_overlay/bin/python}"
SEED="${SUCC_GRAPH_JUMP_SEED:-1985}"
OUTPUT_DIR="${SUCC_GRAPH_JUMP_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/source_clamped_latent_graph_jump_process_v38/seed_${SEED}}"
...
PREREGISTRATION="$SCRIPT_DIR/source_clamped_latent_graph_jump_process_v38_preregistration.json"
...
for path in \
  ...
  "$PREREGISTRATION"; do
  [[ -f "$path" ]] || { echo "ERROR: missing B38 input: $path" >&2; exit 2; }
done

if [[ -f "$OUTPUT_DIR/summary.json" && "${SUCC_GRAPH_JUMP_FORCE:-0}" != "1" ]]; then
  echo "ERROR: completed B38 result exists: $OUTPUT_DIR/summary.json" >&2
  exit 2
fi
...
exec "$PYTHON_BIN" "$SCRIPT_DIR/source_clamped_latent_graph_jump_process.py" \
