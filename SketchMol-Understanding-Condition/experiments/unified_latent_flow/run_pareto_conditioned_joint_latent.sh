#!/usr/bin/env bash
# B33: frozen Pareto-conditioned direct latent sampling on fresh energy sources.
...
PYTHON_BIN="${SUCC_PARETO_JOINT_PYTHON:-/home/bdong/.venvs/molscribe_overlay/bin/python}"
OUTPUT_DIR="${SUCC_PARETO_JOINT_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/pareto_conditioned_joint_latent_v33/seed_1951}"
DATASET_DIR="${SUCC_PARETO_JOINT_DATASET_DIR:-$SHARED_PROJECT_DIR/outputs/unified_smiles_generator_joint_v2/dataset}"
REPRESENTATION_DIR="${SUCC_PARETO_JOINT_REPRESENTATION_DIR:-$SHARED_PROJECT_DIR/outputs/graph_latent_autoencoder_v2/seed_1725}"
FRAGMENT_DIR="${SUCC_PARETO_JOINT_FRAGMENT_DIR:-$SHARED_PROJECT_DIR/outputs/latent_fragment_attachment_kernel_v24/cpu_seed_1761}"
B31_DIR="${SUCC_PARETO_JOINT_B31_DIR:-$SHARED_PROJECT_DIR/outputs/assay_joint_site_token_latent_v31/seed_1931}"
B32_DIR="${SUCC_PARETO_JOINT_B32_DIR:-$SHARED_PROJECT_DIR/outputs/structure_constrained_joint_latent_v32/seed_1941}"
...
PREREGISTRATION="$SCRIPT_DIR/pareto_conditioned_joint_latent_v33_preregistration.json"
...
for path in \
  ...
  "$DRD2_ORACLE" \
  "$PREREGISTRATION"; do
  [[ -f "$path" ]] || { echo "ERROR: missing B33 input: $path" >&2; exit 2; }
done

[[ ! -f "$OUTPUT_DIR/summary.json" ]] || {
  echo "ERROR: completed B33 result exists: $OUTPUT_DIR/summary.json" >&2
  exit 2
}
...
exec "$PYTHON_BIN" "$SCRIPT_DIR/pareto_conditioned_joint_latent.py" \
