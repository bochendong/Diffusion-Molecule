#!/usr/bin/env bash
# Run one frozen-B41 valid-terminal latent graph-jump signal.
...
PYTHON_BIN="${SUCC_VALID_TERMINAL_PYTHON:-/home/bdong/.venvs/molscribe_overlay/bin/python}"
SEED="${SUCC_VALID_TERMINAL_SEED:-1991}"
OUTPUT_DIR="${SUCC_VALID_TERMINAL_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/valid_terminal_molecule_latent_jump_v1/seed_${SEED}}"
...
PREREGISTRATION="$SCRIPT_DIR/valid_terminal_molecule_latent_jump_v1_preregistration.json"
...
for path in \
  ...
  "$PREREGISTRATION"; do
  [[ -f "$path" ]] || { echo "ERROR: missing valid-terminal input: $path" >&2; exit 2; }
done

if [[ -f "$OUTPUT_DIR/summary.json" && "${SUCC_VALID_TERMINAL_FORCE:-0}" != "1" ]]; then
  echo "ERROR: completed valid-terminal result exists: $OUTPUT_DIR/summary.json" >&2
  exit 2
fi
...
exec "$PYTHON_BIN" "$SCRIPT_DIR/valid_terminal_molecule_latent_jump.py" \
