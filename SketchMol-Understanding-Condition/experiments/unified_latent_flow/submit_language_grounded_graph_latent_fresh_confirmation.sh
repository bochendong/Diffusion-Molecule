...
LOG_DIR="${SUCC_LANG_FRESH_LOG_DIR:-$SHARED_REPO_DIR/SketchMol-Understanding-Condition/logs/language_grounded_graph_latent_fresh_v2}"
...
property_job="$(sbatch --parsable \
  --job-name=uca-lang-fresh-property \
  ...
  --mail-user="$MAIL_USER" --mail-type=FAIL \
  --output="$LOG_DIR/uca-lang-fresh-property-%j.log" \
...
evaluate_job="$(sbatch --parsable \
  ...
  --output="$LOG_DIR/uca-lang-fresh-eval-%j.log" \
...
echo "result=$SHARED_REPO_DIR/SketchMol-Understanding-Condition/outputs/language_grounded_graph_latent_fresh_v2/seed_2011/evaluation/summary.json"
