#!/usr/bin/env bash
# Train/evaluate one CPU-only exact-20 compositional closed-transaction VQ signal.
...
PYTHON_BIN="${SUCC_COMPOSITIONAL_VQ_PYTHON:-/home/bdong/.venvs/molscribe_overlay/bin/python}"
SEED="${SUCC_COMPOSITIONAL_VQ_SEED:-2007}"
OUTPUT_DIR="${SUCC_COMPOSITIONAL_VQ_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/compositional_closed_transaction_vq_flow_v1/seed_${SEED}}"
...
PREREGISTRATION="$SCRIPT_DIR/compositional_closed_transaction_vq_flow_v1_preregistration.json"
...
for path in \
  ...
  "$PREREGISTRATION"; do
  [[ -f "$path" ]] || { echo "ERROR: missing compositional-VQ input: $path" >&2; exit 2; }
done

if [[ -f "$OUTPUT_DIR/summary.json" && "${SUCC_COMPOSITIONAL_VQ_FORCE:-0}" != "1" ]]; then
  echo "ERROR: completed compositional-VQ result exists: $OUTPUT_DIR/summary.json" >&2
  exit 2
fi
...
exec "$PYTHON_BIN" "$SCRIPT_DIR/compositional_closed_transaction_vq_flow.py" \
