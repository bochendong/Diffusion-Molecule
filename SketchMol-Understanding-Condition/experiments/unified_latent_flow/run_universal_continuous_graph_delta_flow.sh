#!/usr/bin/env bash
# Run one CPU-only exact-20 universal continuous graph-delta signal.
...
PYTHON_BIN="${SUCC_UNIVERSAL_CONTINUOUS_PYTHON:-/home/bdong/.venvs/molscribe_overlay/bin/python}"
SEED="${SUCC_UNIVERSAL_CONTINUOUS_SEED:-2015}"
OUTPUT_DIR="${SUCC_UNIVERSAL_CONTINUOUS_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/universal_continuous_graph_delta_flow_v1/seed_${SEED}}"
...
PREREGISTRATION="$SCRIPT_DIR/universal_continuous_graph_delta_flow_v1_preregistration.json"
...
for path in \
  ...
  "$PREREGISTRATION"; do
  [[ -f "$path" ]] || { echo "ERROR: missing universal-continuous input: $path" >&2; exit 2; }
done

if [[ -f "$OUTPUT_DIR/summary.json" && "${SUCC_UNIVERSAL_CONTINUOUS_FORCE:-0}" != "1" ]]; then
  echo "ERROR: completed universal-continuous result exists: $OUTPUT_DIR/summary.json" >&2
  exit 2
fi
...
exec "$PYTHON_BIN" "$SCRIPT_DIR/universal_continuous_graph_delta_flow.py" \
