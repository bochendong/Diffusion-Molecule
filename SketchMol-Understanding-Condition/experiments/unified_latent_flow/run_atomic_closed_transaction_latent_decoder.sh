#!/usr/bin/env bash
# Train and evaluate one exact-20 atomic closed-transaction latent signal.
...
PYTHON_BIN="${SUCC_ATOMIC_TRANSACTION_PYTHON:-/home/bdong/.venvs/molscribe_overlay/bin/python}"
SEED="${SUCC_ATOMIC_TRANSACTION_SEED:-2005}"
OUTPUT_DIR="${SUCC_ATOMIC_TRANSACTION_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/atomic_closed_transaction_latent_decoder_v1/seed_${SEED}}"
...
PREREGISTRATION="$SCRIPT_DIR/atomic_closed_transaction_latent_decoder_v1_preregistration.json"
...
for path in \
  ...
  "$PREREGISTRATION"; do
  [[ -f "$path" ]] || { echo "ERROR: missing atomic-transaction input: $path" >&2; exit 2; }
done

if [[ -f "$OUTPUT_DIR/summary.json" && "${SUCC_ATOMIC_TRANSACTION_FORCE:-0}" != "1" ]]; then
  echo "ERROR: completed atomic-transaction result exists: $OUTPUT_DIR/summary.json" >&2
  exit 2
fi
...
exec "$PYTHON_BIN" "$SCRIPT_DIR/atomic_closed_transaction_latent_decoder.py" \
