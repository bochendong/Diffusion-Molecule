#!/usr/bin/env bash
# Run one preregistered common-LLM latent-operator comparison arm.
...
DEP_OVERLAY="${SUCC_LLM_LATENT_DEP_OVERLAY:-/scratch/bdong/venvs/uca_common_llm_overlay}"
ARM="${SUCC_LLM_LATENT_ARM:?Set SUCC_LLM_LATENT_ARM}"
...
SFT_ADAPTER_DIR="${SUCC_LLM_LATENT_SFT_ADAPTER_DIR:-$SHARED_PROJECT_DIR/outputs/unified_constraint_agent_common_llm_pilot_v1/model/seed_1703/adapter}"
PREREGISTRATION="$SCRIPT_DIR/llm_latent_operator_signal_v1_preregistration.json"

case "$ARM" in
  property_mlp|base_frozen|sft_frozen|sft_lora) ;;
  *) echo "ERROR: unsupported LLM latent-operator arm: $ARM" >&2; exit 2 ;;
esac
...
for path in \
  ...
  "$PREREGISTRATION"; do
  [[ -f "$path" ]] || { echo "ERROR: missing LLM latent-operator input: $path" >&2; exit 2; }
done

if [[ -f "$OUTPUT_DIR/summary.json" && "${SUCC_LLM_LATENT_FORCE:-0}" != "1" ]]; then
  echo "ERROR: completed LLM latent-operator arm exists: $OUTPUT_DIR/summary.json" >&2
  exit 2
fi
...
exec "$PYTHON_BIN" "$SCRIPT_DIR/llm_latent_operator_signal.py" \
  --arm "$ARM" \
