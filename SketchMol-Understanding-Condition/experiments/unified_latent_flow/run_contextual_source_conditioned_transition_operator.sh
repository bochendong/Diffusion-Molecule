#!/usr/bin/env bash
# Run one CPU-only source-conditioned effect operator on the frozen flow.
...
PYTHON_BIN="${SUCC_CONTEXTUAL_OPERATOR_PYTHON:-/home/bdong/.venvs/molscribe_overlay/bin/python}"
SEED="${SUCC_CONTEXTUAL_OPERATOR_SEED:-2019}"
OUTPUT_DIR="${SUCC_CONTEXTUAL_OPERATOR_OUTPUT_DIR:-$SHARED_PROJECT_DIR/outputs/contextual_source_conditioned_transition_operator_v1/seed_${SEED}}"
...
BALANCED_DIR="$SHARED_PROJECT_DIR/outputs/property_aligned_balanced_transaction_transport_v1/seed_2011"
UNIVERSAL_DIR="$SHARED_PROJECT_DIR/outputs/universal_continuous_graph_delta_flow_v1/seed_2015"
PROBE_DIR="$SHARED_PROJECT_DIR/outputs/compositional_closed_reaction_template_probe_v1"
PREREGISTRATION="$SCRIPT_DIR/contextual_source_conditioned_transition_operator_v1_preregistration.json"
...
for path in \
  ...
  "$BALANCED_DIR/summary.json" \
  "$UNIVERSAL_DIR/universal_continuous_graph_delta_flow.pt" \
  "$UNIVERSAL_DIR/summary.json" \
  "$PREREGISTRATION"; do
  [[ -f "$path" ]] || { echo "ERROR: missing contextual-operator input: $path" >&2; exit 2; }
done

if [[ -f "$OUTPUT_DIR/summary.json" && "${SUCC_CONTEXTUAL_OPERATOR_FORCE:-0}" != "1" ]]; then
  echo "ERROR: completed contextual-operator result exists: $OUTPUT_DIR/summary.json" >&2
  exit 2
fi
...
exec "$PYTHON_BIN" "$SCRIPT_DIR/contextual_source_conditioned_transition_operator.py" \
  ...
  --balanced-summary "$BALANCED_DIR/summary.json" \
  --universal-checkpoint "$UNIVERSAL_DIR/universal_continuous_graph_delta_flow.pt" \
  --universal-summary "$UNIVERSAL_DIR/summary.json" \
