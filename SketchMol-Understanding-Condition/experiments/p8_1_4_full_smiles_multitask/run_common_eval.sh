...
JOINT_ROOT="${P814_JOINT_ROOT:-$PROJECT_DIR/outputs/unified_smiles_generator_joint_v2}"
BASE_CHECKPOINT="${P814_BASE_CHECKPOINT:-$PROJECT_DIR/outputs/direct_smiles_denovo_2p7p_v2_group_rl_v1/direct_smiles_model_group_rl/direct_smiles_generator_rl.pt}"
...
sample_one() {
  ...
  "$PYTHON_BIN" "$UNIFIED_DIR/unified_smiles_generator.py" sample \
    ...
    --method "p8_1_4_${ROUND_NAME}" --decoding-mode sample --num-samples 20 \
...
sample_one denovo "$P6_ROOT/data/denovo_hard_gate.csv" "$DIRECT_ROOT/eval_condition_features_hf_vlm" 1814
sample_one edit "$P6_ROOT/data/edit_table1_gate.csv" "$JOINT_ROOT/feature_variants/validation_condition_features_hf_vlm" 2814
...
for budget in 1 8 20; do
  "$PYTHON_BIN" "$PROJECT_DIR/scripts/evaluate_moledit_table1_anyk.py" \
    ...
    --model-name "p8_1_4_${ROUND_NAME}_any${budget}" --task-filter table1 --missing-oracle-policy fail
done
"$PYTHON_BIN" "$PROJECT_DIR/scripts/evaluate_moledit_table1_anyk.py" \
  --reference "$P6_ROOT/data/edit_table1_gate.csv" --candidates "$OUTPUT_ROOT/eval/edit/candidates.csv" \
  --output-dir "$OUTPUT_ROOT/eval/edit/candidate20" --candidate-limit 20 --aggregation candidate \
  --model-name "p8_1_4_${ROUND_NAME}_candidate20" --task-filter table1 --missing-oracle-policy fail
"$PYTHON_BIN" "$SCRIPT_DIR/audit_results.py" --base-checkpoint "$BASE_CHECKPOINT" --checkpoint "$CHECKPOINT" \
