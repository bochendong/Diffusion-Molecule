...
baseline=$(sbatch --parsable --account=def-hup-ab --job-name=p28-fin-base --time=00:45:00 \
  ...
  --export=ALL,P26_SCRIPT_DIR="$P26_DIR",P26_OUTPUT_ROOT="$OUT",P26_GATE_SPLIT=final,P26_EVAL_TAG="$BASELINE_TAG",P26_EVAL_ADAPTER="$INPUT_ADAPTER",P26_EVAL_REQUIRED="$INPUT_ADAPTER/adapter_model.safetensors" \
...
compare=$(sbatch --parsable --account=def-hup-ab --job-name=p28-fin-cmp --time=00:10:00 \
  ...
  "/home/bdong/.venvs/molscribe_overlay/bin/python '$PROJECT/experiments/p25_p23_joint_group_rl/compare_gate.py' --baseline '$OUT/gate/final/$BASELINE_TAG/summary.json' --rl '$OUT/gate/final/editing_protected_pcgrad/summary.json' --output '$OUT/gate/final/comparison.json'")
