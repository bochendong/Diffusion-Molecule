...
CHECKPOINT_NAME="$(basename "$INPUT_ADAPTER")"
CHECKPOINT_STEP="${CHECKPOINT_NAME#checkpoint-}"
BASELINE_TAG="p24_ckpt${CHECKPOINT_STEP}"
OUT="${P28_OUTPUT_ROOT:-$PROJECT/outputs/p28_p24_rl_method_sweep/checkpoint_${CHECKPOINT_STEP}_seed_28001}"
...
baseline=$(sbatch --parsable --account=def-hup-ab --job-name="p28-c${CHECKPOINT_STEP}-base" --time=00:45:00 \
  ...
  --export=ALL,P26_SCRIPT_DIR="$P26_DIR",P26_OUTPUT_ROOT="$OUT",P26_EVAL_TAG="$BASELINE_TAG",P26_EVAL_ADAPTER="$INPUT_ADAPTER",P26_EVAL_REQUIRED="$INPUT_ADAPTER/adapter_model.safetensors" \
...
for variant in vanilla_paired_grpo decoupled_no_pcgrad editing_protected_pcgrad; do
  ...
  compare_jobs[$variant]=$(sbatch --parsable --account=def-hup-ab --job-name="p28-c-${variant:0:4}" --time=00:10:00 \
    ...
    "/home/bdong/.venvs/molscribe_overlay/bin/python '$PROJECT/experiments/p25_p23_joint_group_rl/compare_gate.py' --baseline '$OUT/gate/dev/$BASELINE_TAG/summary.json' --rl '$OUT/gate/dev/$variant/summary.json' --output '$OUT/gate/dev/comparison_${variant}.json'")
done
