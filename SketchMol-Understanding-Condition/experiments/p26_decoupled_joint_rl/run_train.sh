...
for path in "$P251/PREPARED" "$P23/data/train.sft.jsonl" \
...
"$PY" "$SCRIPT_DIR/train_decoupled_joint_rl.py" \
  ...
  --input-adapter "$P23/model/stage1_v2/adapter" --output-dir "$OUT/model/p26" \
  ...
  --checkpoint-every 10 --seed 26001
sha256sum "$OUT/model/p26/adapter/adapter_model.safetensors" > "$OUT/model/p26/FROZEN.sha256"
