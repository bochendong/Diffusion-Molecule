...
for path in "$REQUIRED" "$P251/data/gates/dev.jsonl" "$ADAPTER/adapter_model.safetensors" \
...
"$PY" "$PROJECT/experiments/p25_p23_joint_group_rl/evaluate_frozen_gate.py" \
  --gate-jsonl "$P251/data/gates/dev.jsonl" --base-model "$BASE" \
  --adapter-dir "$ADAPTER" --output-dir "$OUT/gate/dev/$TAG" \
  --method "$TAG" --repeats 4 --batch-size 8 --seed 251251
touch "$OUT/GATE_DEV_${TAG}_COMPLETE"
