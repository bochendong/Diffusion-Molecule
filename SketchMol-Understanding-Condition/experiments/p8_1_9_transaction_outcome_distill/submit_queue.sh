...
pre="$(sbatch "${common[@]}" --time="${P819_PRECOMPUTE_TIME:-00:10:00}" --job-name=p819-pre-s7 --output="$LOG/pre-%j.log" --wrap="bash '$SCRIPT_DIR/run_precompute.sh'")"; pre="${pre%%;*}"
r1="$(sbatch "${common[@]}" --dependency="afterany:$pre" --job-name=p819-r1-s7 --output="$LOG/r1-%j.log" --wrap="bash '$SCRIPT_DIR/run_round.sh' r1")"; r1="${r1%%;*}"
...
printf 'P8.1.9 PRE=%s\nP8.1.9 R1=%s dependency=afterany:%s\nP8.1.9 R2=%s dependency=afterany:%s\n' "$pre" "$r1" "$pre" "$r2" "$r1"
