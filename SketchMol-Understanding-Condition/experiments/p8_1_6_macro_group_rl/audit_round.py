#!/usr/bin/env python3
"""Audit unified checkpoint structure and identity-copy behavior."""
...
def main() -> int:
    ...
    valid=identity=0
    for row in rows:
        smi=str(row.get("direct_candidate_canonical_smiles") or row.get("generated_smiles") or "").strip();src=str(row.get("source_smiles") or "").strip();valid+=int(bool(smi));identity+=int(bool(smi) and smi==src)
    payload={"protocol":"p8_1_6_unified_audit_v1","one_checkpoint":True,"one_decoder":"decoder.layers.0.self_attn.in_proj_weight" in state,"one_output_softmax":"output.weight" in state,"same_smiles_vocabulary":base.get("vocab")==ck.get("vocab"),"vocab_size":len(ck.get("vocab",{})),"new_output_tokens":len(ck.get("vocab",{}))-len(base.get("vocab",{})),"condition_layout":args.get("condition_layout"),"explicit_task_token":args.get("condition_layout")=="unified","reward_aggregation":args.get("reward_aggregation"),"expected_reward_aggregation":a.expected_aggregation,"source_copy_pointer":bool(config.get("source_copy_aware",False)),"router":False,"interpreter":False,"materializer":False,"property_rerank":False,"candidate_rows":len(rows),"candidate_validity":valid/max(len(rows),1),"identity_copy_rate":identity/max(valid,1),"nonidentity_valid_rate":(valid-identity)/max(len(rows),1)}
