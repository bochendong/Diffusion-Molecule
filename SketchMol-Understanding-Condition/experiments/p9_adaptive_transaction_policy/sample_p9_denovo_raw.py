#!/usr/bin/env python3
"""Write the complete raw de-novo pool from a P9 shared checkpoint."""
    ...
    with torch.no_grad():
        for row_index, row in enumerate(rows):
            ...
            for candidate_index, ids in enumerate(generated.tolist()):
                tokens = vocab.decode(ids[1:])
                raw = unified.detokenize_smiles(tokens)
                ...
                item.update(
                    {
                        ...
                        "method": "p9_adaptive_transaction_policy",
            ...
            if (row_index + 1) % 20 == 0 or row_index + 1 == len(rows):
                print(f"[p9-denovo] {row_index + 1}/{len(rows)} rows", flush=True)
    p6.write_rows(args.output_csv, output)
    total = len(rows) * int(args.num_samples)
    summary = {
        "protocol": "p9_raw_denovo_shared_checkpoint",
        "eval_rows": len(rows),
        "num_samples": int(args.num_samples),
        "valid_candidates": valid,
        "candidate_validity": valid / max(total, 1),
        "candidate_output_csv": str(args.output_csv),
    }
