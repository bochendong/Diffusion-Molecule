...
MODALITY_MEM="${SUCC_UNIFIED_MODALITY_SLURM_MEM:-96G}"
CPUS="${SUCC_UNIFIED_PIPELINE_SLURM_CPUS:-8}"
GPU="${SUCC_UNIFIED_PIPELINE_SLURM_GPUS:-nvidia_h100_80gb_hbm3_3g.40gb:1}"
...
feat_train_out="$(submit_gpu "succ-unified-feat-tr" "afterok:$prep_job" "SUCC_UNIFIED_FEATURE_VARIANTS_RUN_TRAIN=1 SUCC_UNIFIED_FEATURE_VARIANTS_RUN_EVAL=0 bash '$SCRIPT_DIR/run_unified_smiles_generator_feature_variants.sh'")"
echo "$feat_train_out"
feat_train_job="$(echo "$feat_train_out" | sed -n 's/Submitted batch job \([0-9][0-9]*\).*/\1/p')"

feat_eval_out="$(submit_gpu "succ-unified-feat-ev" "afterok:$prep_job" "SUCC_UNIFIED_FEATURE_VARIANTS_RUN_TRAIN=0 SUCC_UNIFIED_FEATURE_VARIANTS_RUN_EVAL=1 bash '$SCRIPT_DIR/run_unified_smiles_generator_feature_variants.sh'")"
echo "$feat_eval_out"
