...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if "moledit_table1" in tasks:
        ...
        records.extend(run_moledit_table1_benchmarks(args))
...
def run_moledit_table1_benchmarks(args: argparse.Namespace) -> list[dict[str, object]]:
    assert args.candidate_prediction_csv is not None
    assert args.moledit_reference_csv is not None
    records = []
    for budget in parse_int_list(str(args.moledit_budgets)):
        task_dir = args.output_dir / "moledit_table1" / f"n{budget}"
        task_dir.mkdir(parents=True, exist_ok=True)
        method_name = f"{args.method_name}_moledit_table1_n{budget}"
        selected_csv = task_dir / f"unified_moledit_table1_selected_n{budget}.csv"
        selection_summary = select_moledit_candidates(
            ...
            selection_mode=str(args.moledit_selection_mode),
        )
...
def candidate_sort_key(row: Mapping[str, object]) -> tuple[int, str]:
    rank = parse_float(row.get("candidate_rank"), default=1_000_000.0)
