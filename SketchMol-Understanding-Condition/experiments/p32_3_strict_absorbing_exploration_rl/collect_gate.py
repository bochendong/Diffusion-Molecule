#!/usr/bin/env python3
"""Apply the preregistered P32.3 editing-improvement gate."""
...
def assess(candidate, direct, step0, rescues):
    checks = {
        mode: {
            ...
            "valid_delta_vs_step0": candidate[mode]["valid_macro"] - step0[mode]["valid_macro"],
            "strict_rescues": int(rescues[mode]),
        }
        for mode in MODES
    }
    passed = (
        checks["de_novo"]["strict_delta_vs_direct"] > 0.0
        and checks["edit"]["strict_delta_vs_direct"] > 0.0
        and checks["de_novo"]["strict_delta_vs_step0"] >= -1e-12
        and checks["edit"]["strict_delta_vs_step0"] > 0.0
        and all(checks[mode]["valid_delta_vs_step0"] >= -1e-12 for mode in MODES)
        and checks["edit"]["strict_rescues"] >= 1
    )
...
def main() -> int:
    ...
    selected = max(
        passed_steps,
        key=lambda step: (
            candidates[str(step)]["metrics"]["edit"]["strict_macro"],
            candidates[str(step)]["metrics"]["de_novo"]["strict_macro"],
            -step,
        ),
        default=None,
    )
    result = {
        ...
        "decision": "PROMOTE_P32_3" if selected is not None else "STOP_P32_3",
    }
