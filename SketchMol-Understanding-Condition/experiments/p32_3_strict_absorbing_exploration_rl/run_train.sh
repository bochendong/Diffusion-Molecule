...
"$PY" "$SCRIPT_DIR/train_strict_absorbing_rl.py" \
  --train-jsonl "$P321/data/train.jsonl" \
  --support-jsonl "$OUT/support_audit/rows.jsonl" \
  --base-model Qwen/Qwen2.5-1.5B-Instruct \
  --input-adapter "$P322/model/checkpoint-030/adapter" \
  --output-dir "$OUT/model" --pairs 20 \
  --denovo-group-size 8 --edit-second-samples 4 --minimum-supported-edit 5 \
  --max-actions 16 --site-limit 24 --score-batch-size 4 --max-length 768 \
  --temperature 1.5 --learning-rate 1e-6 --grad-clip 1.0 \
  --checkpoint-steps 5,10,20 --seed 32301
