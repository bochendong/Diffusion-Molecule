#!/usr/bin/env bash
# Run a group-relative RL fine-tune for the v2 direct-SMILES 2p-7p model.
...
BASE_OUTPUT_DIR="${SUCC_DIRECT_GROUP_RL_BASE_OUTPUT_DIR:-SketchMol-Understanding-Condition/outputs/direct_smiles_denovo_2p7p_v2_mixed_condition}"
OUTPUT_DIR="${SUCC_DIRECT_GROUP_RL_OUTPUT_DIR:-SketchMol-Understanding-Condition/outputs/direct_smiles_denovo_2p7p_v2_group_rl_v1}"
TRAIN_ROWS_CSV="${SUCC_DIRECT_GROUP_RL_TRAIN_ROWS_CSV:-$BASE_OUTPUT_DIR/denovo_2p7p_train_rows.csv}"
EVAL_ROWS_CSV="${SUCC_DIRECT_GROUP_RL_EVAL_ROWS_CSV:-$BASE_OUTPUT_DIR/denovo_2p7p_eval_rows.csv}"
TRAIN_FEATURES_DIR="${SUCC_DIRECT_GROUP_RL_TRAIN_FEATURES_DIR:-$BASE_OUTPUT_DIR/train_condition_features_hf_vlm}"
...
RL_BATCH_SIZE="${SUCC_DIRECT_GROUP_RL_BATCH_SIZE:-8}"
RL_EVAL_BATCH_SIZE="${SUCC_DIRECT_GROUP_RL_EVAL_BATCH_SIZE:-32}"
RL_LR="${SUCC_DIRECT_GROUP_RL_LR:-1e-6}"
RL_WEIGHT_DECAY="${SUCC_DIRECT_GROUP_RL_WEIGHT_DECAY:-1e-4}"
RL_GRAD_CLIP="${SUCC_DIRECT_GROUP_RL_GRAD_CLIP:-1.0}"
RL_ROLLOUTS_PER_PROMPT="${SUCC_DIRECT_GROUP_RL_ROLLOUTS_PER_PROMPT:-16}"
RL_PARALLEL_SAMPLES="${SUCC_DIRECT_GROUP_RL_PARALLEL_SAMPLES:-4}"
RL_MAX_PARALLEL_SEQUENCES="${SUCC_DIRECT_GROUP_RL_MAX_PARALLEL_SEQUENCES:-512}"
RL_MAX_NEW_TOKENS="${SUCC_DIRECT_GROUP_RL_MAX_NEW_TOKENS:-96}"
RL_TEMPERATURE="${SUCC_DIRECT_GROUP_RL_TEMPERATURE:-0.85}"
RL_TOP_K="${SUCC_DIRECT_GROUP_RL_TOP_K:-40}"
RL_TOP_P="${SUCC_DIRECT_GROUP_RL_TOP_P:-0.95}"
RL_REPETITION_PENALTY="${SUCC_DIRECT_GROUP_RL_REPETITION_PENALTY:-1.15}"
RL_NO_REPEAT_NGRAM_SIZE="${SUCC_DIRECT_GROUP_RL_NO_REPEAT_NGRAM_SIZE:-6}"
RL_MIN_NEW_TOKENS="${SUCC_DIRECT_GROUP_RL_MIN_NEW_TOKENS:-6}"
RL_SFT_WEIGHT="${SUCC_DIRECT_GROUP_RL_SFT_WEIGHT:-1.0}"
RL_ADVANTAGE_MODE="${SUCC_DIRECT_GROUP_RL_ADVANTAGE_MODE:-group_zscore}"
RL_ADVANTAGE_CLIP="${SUCC_DIRECT_GROUP_RL_ADVANTAGE_CLIP:-3.0}"
RL_SEQUENCE_LOGPROB_REDUCTION="${SUCC_DIRECT_GROUP_RL_SEQUENCE_LOGPROB_REDUCTION:-mean}"
RL_REFERENCE_KL_WEIGHT="${SUCC_DIRECT_GROUP_RL_REFERENCE_KL_WEIGHT:-0.05}"
RL_REWARD_VALID_WEIGHT="${SUCC_DIRECT_GROUP_RL_REWARD_VALID_WEIGHT:-0.25}"
RL_REWARD_STRICT_WEIGHT="${SUCC_DIRECT_GROUP_RL_REWARD_STRICT_WEIGHT:-2.0}"
RL_REWARD_DISTANCE_WEIGHT="${SUCC_DIRECT_GROUP_RL_REWARD_DISTANCE_WEIGHT:-0.05}"
RL_REWARD_DISTANCE_CLIP="${SUCC_DIRECT_GROUP_RL_REWARD_DISTANCE_CLIP:-10.0}"
DEVICE="${SUCC_DEVICE:-auto}"
SEED="${SUCC_DIRECT_GROUP_RL_SEED:-7}"

BENCHMARK_MAX_NEW_TOKENS="${SUCC_DIRECT_GROUP_RL_BENCHMARK_MAX_NEW_TOKENS:-96}"
BENCHMARK_TEMPERATURE="${SUCC_DIRECT_GROUP_RL_BENCHMARK_TEMPERATURE:-0.85}"
BENCHMARK_TOP_K="${SUCC_DIRECT_GROUP_RL_BENCHMARK_TOP_K:-40}"
BENCHMARK_TOP_P="${SUCC_DIRECT_GROUP_RL_BENCHMARK_TOP_P:-0.95}"
BENCHMARK_NUM_SAMPLES="${SUCC_DIRECT_GROUP_RL_BENCHMARK_NUM_SAMPLES:-128}"
BENCHMARK_PARALLEL_SAMPLES="${SUCC_DIRECT_GROUP_RL_BENCHMARK_PARALLEL_SAMPLES:-8}"
BENCHMARK_MAX_PARALLEL_SEQUENCES="${SUCC_DIRECT_GROUP_RL_BENCHMARK_MAX_PARALLEL_SEQUENCES:-1024}"
BENCHMARK_REPETITION_PENALTY="${SUCC_DIRECT_GROUP_RL_BENCHMARK_REPETITION_PENALTY:-1.15}"
BENCHMARK_NO_REPEAT_NGRAM_SIZE="${SUCC_DIRECT_GROUP_RL_BENCHMARK_NO_REPEAT_NGRAM_SIZE:-6}"
BENCHMARK_MIN_NEW_TOKENS="${SUCC_DIRECT_GROUP_RL_BENCHMARK_MIN_NEW_TOKENS:-6}"

mkdir -p "$OUTPUT_DIR" "$MODEL_DIR" "$BENCHMARK_OUTPUT_DIR" "$BENCHMARK_MODEL_DIR"

echo "Direct-SMILES group-relative RL v2 (2p-7p)"
...
if [[ "$RUN_BENCHMARK_AFTER_TRAIN" == "1" ]]; then
  ...
  "$PYTHON_BIN" "$PROJECT_DIR/scripts/evaluate_univideo_image_benchmark.py" \
    ...
    --report-title "SUCC Direct SMILES Group-RL v2 2p-7p Benchmark" \
