...
echo "  admet_batch_size=$BATCH_SIZE"
...
SBATCH_ARGS=(
  ...
  --export=ALL,SUCC_ORACLE_INPUT_CSV="$INPUT_CSV",SUCC_ORACLE_OUTPUT_CSV="$OUTPUT_CSV",SUCC_ORACLE_WORK_DIR="$WORK_DIR",SUCC_ORACLE_MERGE_PROPERTIES_CSV="$MERGE_CSVS",SUCC_ORACLE_ADMET_REQUIRED_PROPERTIES="$ADMET_REQUIRED_PROPERTIES",SUCC_ORACLE_ADMET_BATCH_SIZE="$BATCH_SIZE",SUCC_ORACLE_SKIP_ADMET="$SKIP_ADMET",SUCC_PYTHON_BIN="$PYTHON_BIN",SUCC_ADMET_PYTHON_BIN="$ADMET_PYTHON_BIN"
)
if [[ -n "$PARTITION" ]]; then
