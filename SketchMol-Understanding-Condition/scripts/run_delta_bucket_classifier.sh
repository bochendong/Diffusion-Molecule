#!/usr/bin/env bash
# Run objective plus delta-magnitude bucket classifier ablations.
...
OUTPUT_DIR="${SUCC_OUTPUT_DIR:-SketchMol-Understanding-Condition/outputs/delta_bucket_classifier_mixed_v2}"
CONDITION_FEATURES_DIR="${SUCC_CONDITION_FEATURES_DIR:-}"
FEATURE_ARRAY="${SUCC_FEATURE_ARRAY:-pooled}"

echo "Running delta-bucket classifier"
...
python "$PROJECT_DIR/scripts/train_delta_bucket_classifier.py" \
...
echo "Delta-bucket classifier finished."
