#!/usr/bin/env bash
# Run a conservative RL fine-tune for the v2 direct-SMILES 2p-7p model.
...
EVAL_FEATURES_DIR="${SUCC_DIRECT_RL_EVAL_FEATURES_DIR:-$BASE_OUTPUT_DIR/eval_condition_features_hf_vlm}"
RESUME_CHECKPOINT="${SUCC_DIRECT_RL_RESUME_CHECKPOINT:-$BASE_OUTPUT_DIR/direct_smiles_model/direct_smiles_generator.pt}"
MODEL_DIR="${SUCC_DIRECT_RL_MODEL_DIR:-$OUTPUT_DIR/direct_smiles_model_rl}"
RUN_BENCHMARK_AFTER_TRAIN="${SUCC_DIRECT_RL_RUN_BENCHMARK_AFTER_TRAIN:-1}"
...
RL_EVAL_BATCH_SIZE="${SUCC_DIRECT_RL_EVAL_BATCH_SIZE:-32}"
RL_LR="${SUCC_DIRECT_RL_LR:-2e-6}"
RL_WEIGHT_DECAY="${SUCC_DIRECT_RL_WEIGHT_DECAY:-1e-4}"
RL_GRAD_CLIP="${SUCC_DIRECT_RL_GRAD_CLIP:-1.0}"
RL_ROLLOUTS_PER_PROMPT="${SUCC_DIRECT_RL_ROLLOUTS_PER_PROMPT:-8}"
RL_PARALLEL_SAMPLES="${SUCC_DIRECT_RL_PARALLEL_SAMPLES:-4}"
RL_MAX_PARALLEL_SEQUENCES="${SUCC_DIRECT_RL_MAX_PARALLEL_SEQUENCES:-512}"
RL_MAX_NEW_TOKENS="${SUCC_DIRECT_RL_MAX_NEW_TOKENS:-96}"
RL_TEMPERATURE="${SUCC_DIRECT_RL_TEMPERATURE:-0.85}"
RL_TOP_K="${SUCC_DIRECT_RL_TOP_K:-40}"
RL_TOP_P="${SUCC_DIRECT_RL_TOP_P:-0.95}"
RL_REPETITION_PENALTY="${SUCC_DIRECT_RL_REPETITION_PENALTY:-1.15}"
RL_NO_REPEAT_NGRAM_SIZE="${SUCC_DIRECT_RL_NO_REPEAT_NGRAM_SIZE:-6}"
RL_MIN_NEW_TOKENS="${SUCC_DIRECT_RL_MIN_NEW_TOKENS:-6}"
RL_SFT_WEIGHT="${SUCC_DIRECT_RL_SFT_WEIGHT:-1.0}"
RL_REWARD_VALID_WEIGHT="${SUCC_DIRECT_RL_REWARD_VALID_WEIGHT:-0.25}"
RL_REWARD_STRICT_WEIGHT="${SUCC_DIRECT_RL_REWARD_STRICT_WEIGHT:-2.0}"
RL_REWARD_DISTANCE_WEIGHT="${SUCC_DIRECT_RL_REWARD_DISTANCE_WEIGHT:-0.05}"
RL_REWARD_DISTANCE_CLIP="${SUCC_DIRECT_RL_REWARD_DISTANCE_CLIP:-10.0}"
DEVICE="${SUCC_DEVICE:-auto}"
SEED="${SUCC_DIRECT_RL_SEED:-7}"

BENCHMARK_MAX_NEW_TOKENS="${SUCC_DIRECT_RL_BENCHMARK_MAX_NEW_TOKENS:-96}"
BENCHMARK_TEMPERATURE="${SUCC_DIRECT_RL_BENCHMARK_TEMPERATURE:-0.85}"
BENCHMARK_TOP_K="${SUCC_DIRECT_RL_BENCHMARK_TOP_K:-40}"
BENCHMARK_TOP_P="${SUCC_DIRECT_RL_BENCHMARK_TOP_P:-0.95}"
BENCHMARK_NUM_SAMPLES="${SUCC_DIRECT_RL_BENCHMARK_NUM_SAMPLES:-128}"
BENCHMARK_PARALLEL_SAMPLES="${SUCC_DIRECT_RL_BENCHMARK_PARALLEL_SAMPLES:-8}"
BENCHMARK_MAX_PARALLEL_SEQUENCES="${SUCC_DIRECT_RL_BENCHMARK_MAX_PARALLEL_SEQUENCES:-1024}"
BENCHMARK_REPETITION_PENALTY="${SUCC_DIRECT_RL_BENCHMARK_REPETITION_PENALTY:-1.15}"
BENCHMARK_NO_REPEAT_NGRAM_SIZE="${SUCC_DIRECT_RL_BENCHMARK_NO_REPEAT_NGRAM_SIZE:-6}"
BENCHMARK_MIN_NEW_TOKENS="${SUCC_DIRECT_RL_BENCHMARK_MIN_NEW_TOKENS:-6}"

mkdir -p "$OUTPUT_DIR" "$MODEL_DIR" "$BENCHMARK_OUTPUT_DIR" "$BENCHMARK_MODEL_DIR"

echo "Direct-SMILES RL v2 (2p-7p)"
...
if [[ "$RUN_BENCHMARK_AFTER_TRAIN" == "1" ]]; then
  ...
  "$PYTHON_BIN" "$PROJECT_DIR/scripts/evaluate_univideo_image_benchmark.py" \
    ...
    --report-title "SUCC Direct SMILES RL v2 2p-7p Benchmark" \
  ...
  echo
  echo "Direct-SMILES RL benchmark ready:"
