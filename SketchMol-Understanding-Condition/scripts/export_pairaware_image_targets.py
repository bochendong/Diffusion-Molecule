#!/usr/bin/env python
"""Export pair-aware supervision targets for image encoder v2.1."""
...
def main() -> None:
    ...
    targets_npz = args.output_dir / "pairaware_image_targets.npz"
    np.savez_compressed(
        targets_npz,
        ...
        target_smiles=np.asarray([row.get("target_smiles", "") for row in kept], dtype=object),
    )
...
def _write_index(path: Path, rows: list[dict[str, str]], labels: list[int], label_names: list[str]) -> None:
    fieldnames = ["pair_id", "split", "source_image", "objective", "direction", "label", "property_delta"]
