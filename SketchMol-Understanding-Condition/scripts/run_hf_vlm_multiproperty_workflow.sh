...
BASELINE_CSV="${SUCC_BASELINE_CSV:-$SMMED_DEFAULT_BASELINE_CSV}"
CONDITION_ROWS="${SMMED_CONDITION_ROWS_CSV:-$SMMED_DEFAULT_CONDITION_ROWS}"
...
SKIP_DATASET_BUILD="${SUCC_SKIP_DATASET_BUILD:-1}"
...
if [[ "$SKIP_DATASET_BUILD" != "1" ]]; then
  run_source_neighbor_dataset_build "$DATASET_PROJECT_DIR"
fi
