#!/usr/bin/env bash
# Train/evaluate the MLLM-conditioned direct SMILES generator on de novo OOD.
...
NEGATIVE_ROWS_CSV="${SUCC_DIRECT_OOD_NEGATIVE_ROWS_CSV:-$OUTPUT_DIR/denovo_ood_negative_rows.csv}"
TRAIN_ROWS_CSV="${SUCC_DIRECT_OOD_TRAIN_ROWS_CSV:-$OUTPUT_DIR/denovo_ood_train_rows.csv}"
CANDIDATE_ROWS_CSV="${SUCC_DIRECT_OOD_CANDIDATE_ROWS_CSV:-$OUTPUT_DIR/denovo_ood_candidate_rows.csv}"
TRAIN_FEATURES_DIR="${SUCC_DIRECT_OOD_TRAIN_FEATURES_DIR:-$OUTPUT_DIR/train_condition_features_hf_vlm}"
EVAL_FEATURES_DIR="${SUCC_DIRECT_OOD_EVAL_FEATURES_DIR:-$OUTPUT_DIR/eval_condition_features_hf_vlm}"
MODEL_DIR="${SUCC_DIRECT_OOD_MODEL_DIR:-$OUTPUT_DIR/direct_smiles_model}"
BENCHMARK_OUTPUT_DIR="${SUCC_DIRECT_OOD_BENCHMARK_OUTPUT_DIR:-$OUTPUT_DIR/benchmark_direct_smiles}"
PREDICTION_CSV="${SUCC_DIRECT_OOD_PREDICTION_CSV:-$BENCHMARK_OUTPUT_DIR/direct_smiles_predictions.csv}"
REPORT_TITLE="${SUCC_DIRECT_OOD_REPORT_TITLE:-SUCC Direct SMILES De Novo OOD Benchmark}"
ROWS_PER_SPEC="${SUCC_DIRECT_OOD_EVAL_ROWS_PER_SPEC:-100}"
TRAIN_ROWS_PER_SPEC="${SUCC_DIRECT_OOD_TRAIN_ROWS_PER_SPEC:-800}"
SEED="${SUCC_DIRECT_OOD_SEED:-37}"
FORCE_EXPORT="${SUCC_DIRECT_OOD_FORCE_EXPORT:-0}"
RUN_FEATURE_EXPORT="${SUCC_DIRECT_OOD_RUN_FEATURE_EXPORT:-auto}"
RUN_TRAIN="${SUCC_DIRECT_OOD_RUN_TRAIN:-1}"
RESUME_CHECKPOINT="${SUCC_DIRECT_OOD_RESUME_CHECKPOINT:-}"
EPOCHS="${SUCC_DIRECT_OOD_EPOCHS:-12}"
BATCH_SIZE="${SUCC_DIRECT_OOD_BATCH_SIZE:-128}"
EVAL_BATCH_SIZE="${SUCC_DIRECT_OOD_EVAL_BATCH_SIZE:-128}"
LR="${SUCC_DIRECT_OOD_LR:-3e-4}"
D_MODEL="${SUCC_DIRECT_OOD_D_MODEL:-256}"
...
CONDITION_MIXING_MODE="${SUCC_DIRECT_OOD_CONDITION_MIXING_MODE:-features_only}"
PROPERTY_COUNT_CURRICULUM_SAMPLING="${SUCC_DIRECT_OOD_PROPERTY_COUNT_CURRICULUM_SAMPLING:-0}"
PROPERTY_COUNT_CURRICULUM_LOSS="${SUCC_DIRECT_OOD_PROPERTY_COUNT_CURRICULUM_LOSS:-0}"
PROPERTY_COUNT_CURRICULUM_POWER="${SUCC_DIRECT_OOD_PROPERTY_COUNT_CURRICULUM_POWER:-1.0}"
PROPERTY_COUNT_CURRICULUM_BASELINE="${SUCC_DIRECT_OOD_PROPERTY_COUNT_CURRICULUM_BASELINE:-2.0}"
...
echo "Direct SMILES de novo OOD benchmark"
...
if [[ "$FORCE_EXPORT" == "1" || ! -f "$EVAL_ROWS_CSV" || ! -f "$TRAIN_ROWS_CSV" ]]; then
  EXPORT_ARGS=(
    ...
    --train-rows-per-spec "$TRAIN_ROWS_PER_SPEC"
    --seed "$SEED"
  )
  if [[ -n "$SPEC_JSON" ]]; then
...
echo
echo "Direct SMILES de novo OOD benchmark ready:"
