...
if [[ "$RUN_MUMO" == "1" ]]; then
  if [[ "$RUN_COPY_SANITY" == "1" ]]; then
    run_step "MuMO source/target-copy sanity" submit_copy_sanity \
      mumo \
      "$MUMO_SOURCE_FILE" \
      SketchMol-Understanding-Condition/outputs/external_mumo_official_copy_sanity_v1 \
  ...
  if [[ "$RUN_GRAPH_EDIT" == "1" ]]; then
    run_step "MuMO GraphEditDSL top20" submit_graph_edit \
      mumo \
      "$MUMO_SOURCE_FILE" \
      SketchMol-Understanding-Condition/outputs/external_mumo_official_graph_edit_heuristic_2step_v1 \
...
if [[ "$RUN_CMUMO" == "1" ]]; then
  if [[ "$RUN_COPY_SANITY" == "1" ]]; then
    run_step "C-MuMO source/target-copy sanity" submit_copy_sanity \
      cmumo \
      "$CMUMO_SOURCE_FILE" \
      SketchMol-Understanding-Condition/outputs/external_cmumo_official_copy_sanity_v1 \
  ...
  if [[ "$RUN_GRAPH_EDIT" == "1" ]]; then
    run_step "C-MuMO GraphEditDSL top20 source-only" submit_graph_edit \
      cmumo \
      "$CMUMO_SOURCE_FILE" \
      SketchMol-Understanding-Condition/outputs/external_cmumo_official_graph_edit_heuristic_2step_v1 \
