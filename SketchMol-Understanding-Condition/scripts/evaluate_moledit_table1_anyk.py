...
def main() -> int:
    ...
    for ref_id, ref in references.items():
        ...
        tanimotos = [float(item["source_tanimoto"]) for item in evaluated_candidates if item.get("source_tanimoto") is not None]
        if tanimotos:
        ...
        summary.update(
            {
                ...
                "missing_prediction_rows": max(reference_counts.get(key, 0) - matched_predictions_by_task.get(key, 0), 0),
                "missing_oracle_skipped_rows": skipped_missing_oracle.get(key, 0),
                "property_anyk": _mean(property_hit_counts.get(key, 0), matched_predictions_by_task.get(key, 0)),
...
def load_candidates(path: Path, *, method_filter: str | None, candidate_limit: int) -> dict[str, list[str]]:
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            ...
            smiles = first_value(row, (*PREDICTION_SMILES_COLUMNS, "direct_candidate_canonical_smiles", "direct_candidate_raw_smiles"))
    ...
    for row_id, items in grouped.items():
        ...
        seen = []
        for _, smiles in items[:candidate_limit]:
            seen.append(smiles)
        out[row_id] = seen
    return out
