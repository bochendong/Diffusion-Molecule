...
export SUCC_OOD_LATENT_RERANK_WEIGHT="${SUCC_OOD_LATENT_RERANK_WEIGHT:-1}"
export SUCC_OOD_RANDOM_RERANK_SEED="${SUCC_OOD_RANDOM_RERANK_SEED:-17}"
...
DEPENDENCY="${SUCC_OOD_SLURM_DEPENDENCY:-}"
...
if [[ ! -f "$SUCC_OOD_RESUME_CHECKPOINT" && -z "$DEPENDENCY" ]]; then
...
echo "  methods=$SUCC_OOD_MATERIALIZED_METHODS"
echo "  property_rerank_candidates=$SUCC_OOD_PROPERTY_RERANK_CANDIDATES"
echo "  property_rerank_weight=$SUCC_OOD_PROPERTY_RERANK_WEIGHT"
echo "  strict_rerank_weight=$SUCC_OOD_STRICT_RERANK_WEIGHT"
echo "  latent_rerank_weight=$SUCC_OOD_LATENT_RERANK_WEIGHT"
echo "  random_rerank_seed=$SUCC_OOD_RANDOM_RERANK_SEED"
