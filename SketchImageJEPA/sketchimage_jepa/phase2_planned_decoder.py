"""Phase 2A: JEPA planner latents decoded by a frozen Phase 1 decoder."""
...
def run_phase2_planned_decoder(
    oracle_decoder_dir: str | Path,
    output_dir: str | Path = "outputs/runs/phase2_planned_decoder",
    ...
    pred_latents = planner.predict(eval_conditions, eval_sources)
    decoded = _retag_candidates(decoder.decode(pred_latents, top_k=top_k), origin="phase2_jepa_planned_decoder")
    ...
    metrics.update(
        {
            ...
            "planner_latent_mse": float(np.mean((pred_latents - eval_targets) ** 2)),
            "planner_latent_cosine": _mean_cosine(pred_latents, eval_targets),
        }
    )
    ...
    run_config = {
        "phase": "phase2a_jepa_planner_frozen_oracle_decoder",
        "research_question": "Can a JEPA planner predict latents usable by the frozen Phase 1 decoder?",
        "oracle_decoder_dir": str(oracle_model_dir),
        "oracle_decoder_config": json.loads((oracle_model_dir / "config.json").read_text(encoding="utf-8")),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Phase 2A JEPA planner with a frozen Phase 1 decoder.")
    parser.add_argument("--oracle-decoder-dir", required=True)
    parser.add_argument("--output-dir", default="outputs/runs/phase2_planned_decoder")
...
def _resolve_oracle_model_dir(path: str | Path) -> Path:
    path = Path(path)
    ...
    if (model_dir / "config.json").exists() and (model_dir / "model.pt").exists():
        return model_dir
    raise FileNotFoundError(f"Could not find a saved oracle decoder model under {path}.")
...
def _load_splits(
    ...
    if train_csv or eval_csv:
        ...
        train_examples, eval_examples = load_examples_csv(train_csv), load_examples_csv(eval_csv)
    else:
...
def _retag_candidates(decoded: list[list[Candidate]], origin: str) -> list[list[Candidate]]:
    return [[Candidate(smiles=item.smiles, origin=origin, score=item.score, rank=item.rank) for item in group] for group in decoded]
...
def _load_decoder_train_pool(run_dir: Path) -> set[str]:
    path = run_dir / "train_smiles.csv"
    if not path.exists():
        return set()
    out: set[str] = set()
...
def _decoder_pool_metrics(scores_by_task, decoder_train_pool: set[str]) -> dict[str, float]:
    ...
    return {
        "top1_decoder_train_pool_member": sum(1.0 for score in top1 if _score_smiles_key(score.smiles) in decoder_train_pool) / n,
        "candidate_decoder_train_pool_member_fraction": sum(1.0 for score in all_scores if _score_smiles_key(score.smiles) in decoder_train_pool)
        / max(1, len(all_scores)),
    }


def _append_decoder_pool_membership(predictions_path: Path, decoder_train_pool: set[str]) -> None:
    if not decoder_train_pool:
        return
    ...
    if "decoder_train_pool_member" not in fieldnames:
        fieldnames.append("decoder_train_pool_member")
    for row in rows:
        row["decoder_train_pool_member"] = _score_smiles_key(row.get("candidate_smiles", "")) in decoder_train_pool
...
def _score_smiles_key(smiles: str | None) -> str:
    return canonicalize_smiles(smiles) or (smiles or "")
