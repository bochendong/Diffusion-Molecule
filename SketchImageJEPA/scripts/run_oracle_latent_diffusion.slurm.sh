#!/bin/bash
#SBATCH --job-name=sketchimage-phase1
#SBATCH --account=def-hup-ab
#SBATCH --gpus=nvidia_h100_80gb_hbm3_1g.10gb:1
#SBATCH --mem=32G
#SBATCH --time=04:00:00
#SBATCH --cpus-per-task=4
#SBATCH --output=./sketchimage-phase1-%j.log
#SBATCH --error=./sketchimage-phase1-%j.log
...
echo "molecule_csv=${SKETCHIMAGE_MOLECULE_CSV:-data/example_molecules.csv}"
echo "run_name=${SKETCHIMAGE_RUN_NAME:-phase1_oracle_latent_diffusion_seed${SKETCHIMAGE_SEED:-7}}"
...
bash scripts/run_oracle_latent_diffusion.sh

echo "SketchImage-JEPA Phase 1 Slurm job finished at $(date +"%Y-%m-%dT%H:%M:%S%z")"
