#!/usr/bin/env bash
# Submit Phase 2D oracle-anchored decoder fine-tuning from a Slurm login node.
...
Then rerun with:
  SKETCHIMAGE_PYTHON_BIN=/scratch/bdong/venvs/phystabmol/bin/python bash scripts/submit_phase2_oracle_anchored_decoder.sh
EOF
  exit 2
fi

export SKETCHIMAGE_SEED="${SKETCHIMAGE_SEED:-7}"
export SKETCHIMAGE_RUN_NAME="${SKETCHIMAGE_RUN_NAME:-phase2_oracle_anchored_decoder_2048_seed${SKETCHIMAGE_SEED}}"
...
if [[ ! -e "$SKETCHIMAGE_ORACLE_DECODER_DIR/model.pt" && ! -e "$SKETCHIMAGE_ORACLE_DECODER_DIR/model/model.pt" ]]; then
  echo "ERROR: oracle decoder model not found under $SKETCHIMAGE_ORACLE_DECODER_DIR" >&2
  exit 2
fi
...
echo "Submitting Phase 2D oracle-anchored decoder:"
...
for SLURM_GPUS in "${GPU_CANDIDATES[@]}"; do
  ...
  if sbatch \
    ...
    --output="./outputs/logs/sketchimage-phase2d-%j.log" \
    --error="./outputs/logs/sketchimage-phase2d-%j.log" \
    scripts/run_phase2_oracle_anchored_decoder.slurm.sh; then
    SUBMITTED=1
    break
  fi
done
...
if [[ "$SUBMITTED" != "1" ]]; then
  ...
  echo "Try SKETCHIMAGE_SLURM_GPUS=<available_gpu_name>:1 bash scripts/submit_phase2_oracle_anchored_decoder.sh" >&2
  exit 2
fi
