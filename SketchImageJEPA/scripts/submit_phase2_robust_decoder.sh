#!/bin/bash
# Submit Phase 2B planner-aware decoder fine-tuning.
#
...
Then rerun with:
  SKETCHIMAGE_PYTHON_BIN=/scratch/bdong/venvs/phystabmol/bin/python bash scripts/submit_phase2_robust_decoder.sh
EOF
  exit 2
fi

export SKETCHIMAGE_SEED="${SKETCHIMAGE_SEED:-7}"
export SKETCHIMAGE_RUN_NAME="${SKETCHIMAGE_RUN_NAME:-phase2_robust_decoder_2048_seed${SKETCHIMAGE_SEED}}"
...
echo "Submitting Phase 2B robust planned decoder:"
...
echo "  decoder_finetune_batch_size=$SKETCHIMAGE_DECODER_FINETUNE_BATCH_SIZE"
...
for SLURM_GPUS in "${GPU_CANDIDATES[@]}"; do
  ...
  if sbatch \
    ...
    --output="./outputs/logs/sketchimage-phase2b-%j.log" \
    --error="./outputs/logs/sketchimage-phase2b-%j.log" \
    scripts/run_phase2_robust_decoder.slurm.sh; then
    SUBMITTED=1
    break
  fi
done
...
if [[ "$SUBMITTED" != "1" ]]; then
  ...
  echo "Try SKETCHIMAGE_SLURM_GPUS=<available_gpu_name>:1 bash scripts/submit_phase2_robust_decoder.sh" >&2
  exit 2
fi
