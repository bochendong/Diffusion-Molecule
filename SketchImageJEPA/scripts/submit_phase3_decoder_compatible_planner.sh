#!/bin/bash
# Submit Phase 3A decoder-compatible JEPA planner + frozen Phase 1 decoder.
#
...
Then rerun with:
  SKETCHIMAGE_PYTHON_BIN=/scratch/bdong/venvs/phystabmol/bin/python bash scripts/submit_phase3_decoder_compatible_planner.sh
EOF
  exit 2
fi

export SKETCHIMAGE_SEED="${SKETCHIMAGE_SEED:-7}"
export SKETCHIMAGE_RUN_NAME="${SKETCHIMAGE_RUN_NAME:-phase3_decoder_compatible_planner_2048_seed${SKETCHIMAGE_SEED}}"
...
echo "Submitting Phase 3A decoder-compatible planner:"
...
echo "  norm_loss_weight=$SKETCHIMAGE_TORCH_NORM_LOSS_WEIGHT"
echo "  decoder_compat_loss_weight=$SKETCHIMAGE_TORCH_DECODER_COMPAT_LOSS_WEIGHT"
echo "  decoder_compat_cosine_margin=$SKETCHIMAGE_TORCH_DECODER_COMPAT_COSINE_MARGIN"
echo "  normalize_predictions=$SKETCHIMAGE_TORCH_NORMALIZE_PREDICTIONS"
...
for SLURM_GPUS in "${GPU_CANDIDATES[@]}"; do
  ...
  if sbatch \
    ...
    --output="./outputs/logs/sketchimage-phase3-%j.log" \
    --error="./outputs/logs/sketchimage-phase3-%j.log" \
    scripts/run_phase3_decoder_compatible_planner.slurm.sh; then
    SUBMITTED=1
    break
  fi
done
...
if [[ "$SUBMITTED" != "1" ]]; then
  ...
  echo "Try SKETCHIMAGE_SLURM_GPUS=<available_gpu_name>:1 bash scripts/submit_phase3_decoder_compatible_planner.sh" >&2
  exit 2
fi
