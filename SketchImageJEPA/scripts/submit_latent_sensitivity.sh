#!/usr/bin/env bash
# Submit the latent sensitivity diagnostic from a Slurm login node.
...
Then rerun with:
  SKETCHIMAGE_PYTHON_BIN=/scratch/bdong/venvs/phystabmol/bin/python bash scripts/submit_latent_sensitivity.sh
EOF
  exit 2
fi

export SKETCHIMAGE_SEED="${SKETCHIMAGE_SEED:-7}"
export SKETCHIMAGE_RUN_NAME="${SKETCHIMAGE_RUN_NAME:-phase2_latent_sensitivity_2048_seed${SKETCHIMAGE_SEED}}"
...
if [[ ! -f "$SKETCHIMAGE_PLANNER_RUN_DIR/planner_eval_latents.npy" ]]; then
  echo "ERROR: planner latents not found: $SKETCHIMAGE_PLANNER_RUN_DIR/planner_eval_latents.npy" >&2
  exit 2
fi
if [[ ! -f "$SKETCHIMAGE_CALIBRATED_RUN_DIR/calibrated_eval_latents.npy" ]]; then
  echo "ERROR: calibrated latents not found: $SKETCHIMAGE_CALIBRATED_RUN_DIR/calibrated_eval_latents.npy" >&2
  exit 2
fi
...
echo "Submitting latent sensitivity diagnostic:"
...
for SLURM_GPUS in "${GPU_CANDIDATES[@]}"; do
  ...
  if sbatch \
    ...
    --output="./outputs/logs/sketchimage-latsens-%j.log" \
    --error="./outputs/logs/sketchimage-latsens-%j.log" \
    scripts/run_latent_sensitivity.slurm.sh; then
    SUBMITTED=1
    break
  fi
done
...
if [[ "$SUBMITTED" != "1" ]]; then
  ...
  echo "Try SKETCHIMAGE_SLURM_GPUS=<available_gpu_name>:1 bash scripts/submit_latent_sensitivity.sh" >&2
  exit 2
fi
