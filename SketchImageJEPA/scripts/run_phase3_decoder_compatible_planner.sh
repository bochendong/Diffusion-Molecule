#!/usr/bin/env bash
# Run Phase 3A: decoder-compatible JEPA planner with a frozen Phase 1 decoder.
...
RUN_NAME="${SKETCHIMAGE_RUN_NAME:-phase3_decoder_compatible_planner_2048_seed${SKETCHIMAGE_SEED:-7}}"
...
export SKETCHIMAGE_TORCH_NORM_LOSS_WEIGHT="${SKETCHIMAGE_TORCH_NORM_LOSS_WEIGHT:-2.0}"
export SKETCHIMAGE_TORCH_DECODER_COMPAT_LOSS_WEIGHT="${SKETCHIMAGE_TORCH_DECODER_COMPAT_LOSS_WEIGHT:-4.0}"
export SKETCHIMAGE_TORCH_DECODER_COMPAT_COSINE_MARGIN="${SKETCHIMAGE_TORCH_DECODER_COMPAT_COSINE_MARGIN:-0.78}"
export SKETCHIMAGE_TORCH_NORMALIZE_PREDICTIONS="${SKETCHIMAGE_TORCH_NORMALIZE_PREDICTIONS:-1}"
...
echo "SketchImage-JEPA Phase 3A decoder-compatible planner"
...
echo "  torch_norm_loss_weight=$SKETCHIMAGE_TORCH_NORM_LOSS_WEIGHT"
echo "  torch_decoder_compat_loss_weight=$SKETCHIMAGE_TORCH_DECODER_COMPAT_LOSS_WEIGHT"
echo "  torch_decoder_compat_cosine_margin=$SKETCHIMAGE_TORCH_DECODER_COMPAT_COSINE_MARGIN"
echo "  torch_normalize_predictions=$SKETCHIMAGE_TORCH_NORMALIZE_PREDICTIONS"
...
echo "[2/2] Training decoder-compatible planner and decoding through frozen Phase 1 decoder"
LATENT_ARGS=()
...
NORMALIZE_ARGS=(--no-torch-normalize-predictions)
if [[ "$SKETCHIMAGE_TORCH_NORMALIZE_PREDICTIONS" == "1" ]]; then
  NORMALIZE_ARGS=(--torch-normalize-predictions)
fi

"$PYTHON_BIN" -m sketchimage_jepa.phase3_decoder_compatible_planner \
  ...
  --torch-norm-loss-weight "$SKETCHIMAGE_TORCH_NORM_LOSS_WEIGHT" \
  --torch-decoder-compat-loss-weight "$SKETCHIMAGE_TORCH_DECODER_COMPAT_LOSS_WEIGHT" \
  --torch-decoder-compat-cosine-margin "$SKETCHIMAGE_TORCH_DECODER_COMPAT_COSINE_MARGIN" \
  "${NORMALIZE_ARGS[@]}" \
...
echo
echo "Phase 3A decoder-compatible planner finished: $OUTPUT_DIR"
