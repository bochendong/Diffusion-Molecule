    ...
    case "$variant" in
      ...
      planner_latent_beam_transform_only)
        submit_gpu_variant "$variant" "$seed" \
          ...
          "SKETCHIMAGE_GENERATIVE_NOVELTY_BONUS=0.05"
        ;;
      planner_edit_policy_transform)
        submit_gpu_variant "$variant" "$seed" \
          "SKETCHIMAGE_DECODER_MODE=hybrid_edit_policy_transform" \
          "SKETCHIMAGE_GENERATIVE_SEED_COUNT=64" \
          "SKETCHIMAGE_GENERATIVE_MUTATION_ROUNDS=2" \
          "SKETCHIMAGE_GENERATIVE_CANDIDATES_PER_SEED=16" \
          "SKETCHIMAGE_GENERATIVE_NOVELTY_BONUS=0.05"
        ;;
      planner_edit_policy_transform_only)
        submit_gpu_variant "$variant" "$seed" \
          "SKETCHIMAGE_DECODER_MODE=edit_policy_transform" \
          "SKETCHIMAGE_GENERATIVE_SEED_COUNT=64" \
          "SKETCHIMAGE_GENERATIVE_MUTATION_ROUNDS=2" \
          "SKETCHIMAGE_GENERATIVE_CANDIDATES_PER_SEED=16" \
          "SKETCHIMAGE_GENERATIVE_NOVELTY_BONUS=0.05"
