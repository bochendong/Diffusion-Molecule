...
TORCH_DELTA_LOSS_WEIGHT="${SKETCHIMAGE_TORCH_DELTA_LOSS_WEIGHT:-0.2}"
...
TORCH_HARD_NEGATIVE_LOSS_WEIGHT="${SKETCHIMAGE_TORCH_HARD_NEGATIVE_LOSS_WEIGHT:-0.0}"
TORCH_HARD_NEGATIVE_MARGIN="${SKETCHIMAGE_TORCH_HARD_NEGATIVE_MARGIN:-0.10}"
...
DECODER_MODE="${SKETCHIMAGE_DECODER_MODE:-retrieval}"
GENERATIVE_SEED_COUNT="${SKETCHIMAGE_GENERATIVE_SEED_COUNT:-24}"
GENERATIVE_MUTATION_ROUNDS="${SKETCHIMAGE_GENERATIVE_MUTATION_ROUNDS:-1}"
GENERATIVE_CANDIDATES_PER_SEED="${SKETCHIMAGE_GENERATIVE_CANDIDATES_PER_SEED:-8}"
GENERATIVE_NOVELTY_BONUS="${SKETCHIMAGE_GENERATIVE_NOVELTY_BONUS:-0.05}"
...
if [[ "$BACKEND" == "torch_denoiser" ]]; then
  ...
  echo "  torch_delta_loss_weight=$TORCH_DELTA_LOSS_WEIGHT"
  ...
  echo "  torch_hard_negative_loss_weight=$TORCH_HARD_NEGATIVE_LOSS_WEIGHT"
  echo "  torch_hard_negative_margin=$TORCH_HARD_NEGATIVE_MARGIN"
...
if [[ "$DECODER_MODE" != "retrieval" ]]; then
  echo "  generative_seed_count=$GENERATIVE_SEED_COUNT"
  echo "  generative_mutation_rounds=$GENERATIVE_MUTATION_ROUNDS"
  echo "  generative_candidates_per_seed=$GENERATIVE_CANDIDATES_PER_SEED"
  echo "  generative_novelty_bonus=$GENERATIVE_NOVELTY_BONUS"
fi
...
if [[ "$BACKEND" == "torch_denoiser" ]]; then
  ARGS+=(
    ...
    --torch-delta-loss-weight "$TORCH_DELTA_LOSS_WEIGHT"
    ...
    --torch-hard-negative-loss-weight "$TORCH_HARD_NEGATIVE_LOSS_WEIGHT"
    --torch-hard-negative-margin "$TORCH_HARD_NEGATIVE_MARGIN"
...
ARGS+=(--decoder-mode "$DECODER_MODE")
ARGS+=(--generative-seed-count "$GENERATIVE_SEED_COUNT")
ARGS+=(--generative-mutation-rounds "$GENERATIVE_MUTATION_ROUNDS")
ARGS+=(--generative-candidates-per-seed "$GENERATIVE_CANDIDATES_PER_SEED")
ARGS+=(--generative-novelty-bonus "$GENERATIVE_NOVELTY_BONUS")
