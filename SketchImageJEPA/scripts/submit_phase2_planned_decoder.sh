#!/bin/bash
# Submit Phase 2A JEPA planner + frozen Phase 1 decoder.
#
...
Then rerun with:
  SKETCHIMAGE_PYTHON_BIN=/scratch/bdong/venvs/phystabmol/bin/python bash scripts/submit_phase2_planned_decoder.sh
EOF
  exit 2
fi

export SKETCHIMAGE_SEED="${SKETCHIMAGE_SEED:-7}"
export SKETCHIMAGE_RUN_NAME="${SKETCHIMAGE_RUN_NAME:-phase2_planned_decoder_2048_seed${SKETCHIMAGE_SEED}}"
...
echo "Submitting Phase 2A planned decoder:"
...
for SLURM_GPUS in "${GPU_CANDIDATES[@]}"; do
  ...
  if sbatch \
    ...
    --output="./outputs/logs/sketchimage-phase2-%j.log" \
    --error="./outputs/logs/sketchimage-phase2-%j.log" \
    scripts/run_phase2_planned_decoder.slurm.sh; then
    SUBMITTED=1
    break
  fi
done
...
if [[ "$SUBMITTED" != "1" ]]; then
  ...
  echo "Try SKETCHIMAGE_SLURM_GPUS=<available_gpu_name>:1 bash scripts/submit_phase2_planned_decoder.sh" >&2
  exit 2
fi
