#!/bin/bash
#SBATCH --job-name=sketchimage-latsens
#SBATCH --account=def-hup-ab
#SBATCH --gpus=nvidia_h100_80gb_hbm3_1g.10gb:1
#SBATCH --mem=64G
#SBATCH --time=04:00:00
#SBATCH --cpus-per-task=8
#SBATCH --output=./sketchimage-latsens-%j.log
#SBATCH --error=./sketchimage-latsens-%j.log
...
echo "decoder_dir=${SKETCHIMAGE_DECODER_DIR:-outputs/runs/phase2_robust_decoder_2048_seed${SKETCHIMAGE_SEED:-7}/decoder}"
echo "planner_run_dir=${SKETCHIMAGE_PLANNER_RUN_DIR:-outputs/runs/phase2_robust_decoder_2048_seed${SKETCHIMAGE_SEED:-7}}"
echo "calibrated_run_dir=${SKETCHIMAGE_CALIBRATED_RUN_DIR:-outputs/runs/phase2_calibrated_decoder_2048_seed${SKETCHIMAGE_SEED:-7}}"
...
echo "run_name=${SKETCHIMAGE_RUN_NAME:-phase2_latent_sensitivity_2048_seed${SKETCHIMAGE_SEED:-7}}"
...
bash scripts/run_latent_sensitivity.sh

echo "SketchImage-JEPA latent sensitivity Slurm job finished at $(date +"%Y-%m-%dT%H:%M:%S%z")"
