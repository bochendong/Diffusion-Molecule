#!/bin/bash
#SBATCH --job-name=sketchimage-phase3
#SBATCH --account=def-hup-ab
#SBATCH --gpus=nvidia_h100_80gb_hbm3_1g.10gb:1
#SBATCH --mem=64G
#SBATCH --time=10:00:00
#SBATCH --cpus-per-task=8
#SBATCH --output=./sketchimage-phase3-%j.log
#SBATCH --error=./sketchimage-phase3-%j.log
...
echo "run_name=${SKETCHIMAGE_RUN_NAME:-phase3_decoder_compatible_planner_2048_seed${SKETCHIMAGE_SEED:-7}}"
...
bash scripts/run_phase3_decoder_compatible_planner.sh

echo "SketchImage-JEPA Phase 3A Slurm job finished at $(date +"%Y-%m-%dT%H:%M:%S%z")"
