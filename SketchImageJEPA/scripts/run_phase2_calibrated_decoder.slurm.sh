#!/bin/bash
#SBATCH --job-name=sketchimage-phase2c
#SBATCH --account=def-hup-ab
#SBATCH --gpus=nvidia_h100_80gb_hbm3_1g.10gb:1
#SBATCH --mem=64G
#SBATCH --time=08:00:00
#SBATCH --cpus-per-task=8
#SBATCH --output=./sketchimage-phase2c-%j.log
#SBATCH --error=./sketchimage-phase2c-%j.log
...
echo "decoder_pool_dir=${SKETCHIMAGE_DECODER_POOL_DIR:-outputs/runs/phase1_oracle_latent_ar_2048_seed${SKETCHIMAGE_SEED:-7}}"
...
echo "run_name=${SKETCHIMAGE_RUN_NAME:-phase2_calibrated_decoder_2048_seed${SKETCHIMAGE_SEED:-7}}"
...
bash scripts/run_phase2_calibrated_decoder.sh

echo "SketchImage-JEPA Phase 2C Slurm job finished at $(date +"%Y-%m-%dT%H:%M:%S%z")"
