#!/bin/bash
#SBATCH --job-name=sketchimage-phase2b
#SBATCH --account=def-hup-ab
#SBATCH --gpus=nvidia_h100_80gb_hbm3_1g.10gb:1
#SBATCH --mem=64G
#SBATCH --time=10:00:00
#SBATCH --cpus-per-task=8
#SBATCH --output=./sketchimage-phase2b-%j.log
#SBATCH --error=./sketchimage-phase2b-%j.log
...
echo "run_name=${SKETCHIMAGE_RUN_NAME:-phase2_robust_decoder_2048_seed${SKETCHIMAGE_SEED:-7}}"
...
bash scripts/run_phase2_robust_decoder.sh

echo "SketchImage-JEPA Phase 2B Slurm job finished at $(date +"%Y-%m-%dT%H:%M:%S%z")"
