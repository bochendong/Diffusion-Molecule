#!/usr/bin/env bash
# Submit Route B with SELFIES tokens and the image head, without CLIP alignment.
...
export SKETCHMOL_JOINT_RUN_NAME="${SKETCHMOL_JOINT_RUN_NAME:-sketchmol_compare_joint_selfies_image_only_seed${SEED}}"
...
export SKETCHMOL_JOINT_CLIP_LOSS_WEIGHT="${SKETCHMOL_JOINT_CLIP_LOSS_WEIGHT:-0.0}"
...
echo "SketchMolCompare -> Route B SELFIES image-only joint diffusion"
