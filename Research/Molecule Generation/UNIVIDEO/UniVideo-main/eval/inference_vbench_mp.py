...
def main():
    ...
    # --- 2. Load VBench Data (Standard Only) ---
    if rank == 0:
    ...
    # Create dataset list
    # Since we are NOT using augmented prompts, "prompt_en" is used for everything.
    full_dataset = []
    for info in vbench_data:
        full_dataset.append({
            "dimension": info['dimension'],
            "prompt": info['prompt_en'], # Standard short prompt
        })
    ...
    for i, global_idx in enumerate(sampler):
        ...
        prompt_text = item['prompt'] # Short prompt

        # Output Structure: output_dir / dimension / prompt-0.mp4
        save_dir = os.path.join(args.output_dir, dimension)
        os.makedirs(save_dir, exist_ok=True)

        print(f"[Rank {rank}] Processing ({dimension}): {prompt_text[:30]}...")

        for video_idx in range(args.num_videos):
            filename = f"{prompt_text}-{video_idx}.mp4"
            ...
            pipeline_kwargs = dict(
                prompts=[prompt_text], # Using Standard Prompt
